...
nav_order: 2
permalink: /docs/AppFramework.html
---
...
- [Configure Credentials to Remote Store](#configure-credentials-to-connect-to-remote-store)
  - [Credentials for S3 Remote Storage](#configure-credentials-for-s3-based-remote-storage)
  - [Credentials for Azure Blob Remote Storage](#configure-credentials-for-azure-blob-remote-storage)
    - [Comparing Azure Managed Identity and Workload Identity](#comparing-azure-managed-identity-and-workload-identity)
    - [Azure Blob Access with Managed Identity](#setup-azure-blob-access-with-managed-identity)
    - [Azure Blob Access with Workload Identity](#setup-azure-blob-access-with-azure-workload-identity)
    - [Azure Blob Access with Kubernetes Secrets](#setup-azure-blob-access-with-kubernetes-secrets)
    - [Azure Blob Authorization Recommendations](#azure-blob-authorization-recommendations)
  - [Credentials for GCP Remote Storage](#configure-credentials-for-gcp-bucket-remote-storage)
    - [Comparing GCP Workload Identity and Service Account Keys](#comparing-gcp-workload-identity-and-service-account-keys)
    - [GCP Bucket Access with Workload Identity](#setup-gcp-bucket-access-with-workload-identity)
    - [GCP Bucket Access with Kubernetes Secrets](#setup-gcp-bucket-access-with-kubernetes-secrets)
    - [GCP Bucket Authorization Recommendations](#gcp-bucket-authorization-recommendations)
- [Setup Remote Storage Contents](#setup-remote-storage-contents)
  - [Create Unique Folders](#create-unique-folders)
  - [Upload Apps to Folders](#upload-apps-to-folders)
...
- [Add a persistent storage volume to the Operator pod](#add-a-persistent-storage-volume-to-the-operator-pod)
- [Manual Initiation of App Management](#manual-initiation-of-app-management)
  - [Disabling Automatic Polling](#disabling-automatic-polling)
  - [Manual Triggering of App Checks](#manual-triggering-of-app-checks)
  - [Reinstate Automatic Polling](#reinstate-automatic-polling)
  - [Important Considerations](#important-considerations)
- [App Framework Limitations](#app-framework-limitations)
- [App Framework Troubleshooting](#app-framework-troubleshooting)
  - [App Framework Phase Information](#app-framework-phase-information)
  - [App Framework Bundle Push Status](#app-framework-bundle-push-status)
  - [App is not updating after new file uploaded to remote storage](#app-is-not-updating-after-new-file-uploaded-to-remote-storage)
...
The App Framework detects the Splunk app or add-on archive files available in the App Source locations, and deploys them to the CR instance path for local or cluster use (see [appSources](#appsources)).

The App Framework maintains a checksum for each app or add-on archive file in the App Source location in the form of the Etag of the remote storage object. The app name and checksum is recorded in the CR, and is used to compare the deployed apps to the app archive files in the App Source location. The App Framework will scan for changes to the App Source folders using the polling interval, and deploy any updated apps to the instance. For the App Framework to detect that an app or add-on had changed, the updated app must use the same archive file name as the previously deployed one.
...
1) Utilizing the App Framework requires one of the following remote storage providers:
...
## Configure credentials to connect to remote store

### Configure credentials for s3 based remote storage
Configure an IAM role for the Operator and Splunk instance pods using a service account or annotations.
...
Below are multiple options for connecting to Azure Blob remote storage.

#### Comparing Azure Managed Identity and Workload Identity
...
#### Setup Azure Blob Access with Kubernetes Secrets
...
3. **Secure Service Accounts:** Ensure that Kubernetes Service Accounts used with Workload Identity are restricted to only the necessary namespaces and roles.

### Configure credentials for GCP Bucket remote storage

Below are multiple options for connecting to GCP Bucket remote storage.

#### Comparing GCP Workload Identity and Service Account Keys
...
#### Setup GCP Bucket Access with Workload Identity
...
7. **Configure Splunk Operator to Use Workload Identity**
    ...
    - **Update Custom Resource Definition (CRD)**:
      - Ensure that your Splunk Operator's CRD is configured to utilize the Kubernetes Service Account `splunk-operator-sa` for GCS access. See more in the [examples](#examples-of-app-framework-usage).
...
#### Setup GCP Bucket Access with Kubernetes Secrets
...
4. **Configure Splunk Operator to Use the Kubernetes Secret**
    ...
    - **Update Custom Resource Definition (CRD)**:
      - Ensure that your Splunk Operator's CRD references the `gcs-secret` for GCS access. See more in the [examples](#examples-of-app-framework-usage).
...
## Setup Remote Storage Contents

### Create Unique Folders
Create unique folders on the remote storage volume to use as App Source locations. An App Source in each CRD is a folder on the remote storage volume containing a select subset of Splunk apps and add-ons. For example, the network and authentication Splunk Apps can be split into different folders and named `networkApps` and `authApps`.

### Upload Apps to Folders
Copy your Splunk App or Add-on archive files to the unique folders on the remote storage. For example, the Splunk Apps can be located at `bucket-app-framework/Standalone-us/networkAppsLoc/` and `bucket-app-framework/Standalone-us/authAppsLoc/`. All folders should be accessible through the endpoint `https://s3-us-west-2.amazonaws.com` for s3, https://mystorageaccount.blob.core.windows.net for azure blob, and https://storage.googleapis.com for GCP bucket, or a similar endpoint for your system.
...
If app framework is enabled, the Splunk Operator creates a namespace scoped configMap named **splunk-\<namespace\>-manual-app-update**, which is used to manually trigger the app updates. By default, the App Framework polls the remote object storage location for new or changed apps on the remote object storage at the `appsRepoPollIntervalSeconds` interval.

To disable the interval check, and manage app updates manually, see the [Manual initiation of app management](#manual-initiation-of-app-management).

## Examples of App Framework usage
Following section shows examples of using App Framework for all remote data storages. First, the examples for S3 based remote object storage are given, and then the same examples are covered for Azure blob and GCP buckets. The examples in both the cases have lot of commonalities and the places they differ are mainly in the values for `storageType`, `provider` and `endpoint`.
...
Apply the Custom Resource specification: `kubectl apply -f ClusterManager.yaml`
...
Apply the Custom Resource specification: `kubectl apply -f SearchHeadCluster.yaml`

### How to install apps for both local and cluster scopes

If there is a case where the same app needs to be installed for both local (on the search head deployer or cluster manager) and cluster (on the search head peers and indexer peers) scopes, the same remote bucket folder can be used in multiple spec.appRepo.appSources, with different scopes. **Keep in mind that each appSource.name must be unique, but the appSource.location can be the same.** 
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
      - name: splunk-operator
        image: "docker.io/splunk/splunk-operator:3.1.0"
        volumeMounts:
        - mountPath: /opt/splunk/appframework/
          name: app-staging

...
| SHC-75 | Qualify failed captain transfer and revision withdrawal; handle ControllerRevision reuse, in-place readiness handoff, and StatefulSet generation observation | `eb6907ee5`, `44ccac31e`, `3e9e735a7` | LFC-007, STS-014, OBS-001, OBS-002 | EKS-qualified for pre-authorization failure/cancellation, reverse-ordinal rollback, clean Event/log audit, and 321-second stability; STS-014 post-authorization withdrawal remains open |
...
same change that creates its branch. The immediately identified follow-on is
`STS-014`: withdraw the desired revision after replacement authorization while
the target is terminating, starting, or rejoining. That scenario must complete
the already-authorized target to a known state, prohibit a second concurrent
disruption, and then deterministically roll back or queue the new revision.
...
2026-07-28: Created the central SHC-60 through SHC-75 execution registry,
linked implementation commits to stable scenario identifiers, recorded
qualification scope without claiming production readiness, and identified the
post-authorization revision-withdrawal follow-on.

...
| SHC-107 | Qualify long-lived HEC and distributed-search client connections across Kubernetes Service backend replacement | Test-only source `f3ec88026` on `codex/shc-107-persistent-client-qualification` | HLT-014, OPS-006, OPS-011, OBS-001, OBS-003, OBS-005 | Harness, stable reuse, and unplanned active-captain replacement are qualified; remaining disruptive qualification is open. The existing monitors start a new client process per request and cannot prove behavior of an established TCP/TLS flow. SHC-107 reuses HEC and Search Head HTTPS connections, records Search Head identity and connection generation, separates first-attempt transport failure from bounded reconnect recovery and logical failure, and preserves exact final completeness. Seven tests passed 100 repetitions with `make fmt vet` and Kubernetes manifest validation. Stable EKS carried 12 HEC writes on one connection and 25 Search Head requests on one connection. Deleting the selected captain withdrew its endpoint, caused exactly one failed first attempt and one recovered logical request, moved the connection and captain to Search Head 2, and completed all 600 events with zero logical failures or count regressions. Diagnostic runs also proved current Splunkd forces close for absent/default Python and official Python SDK user-agent identities; a neutral capable-client identity permits reuse. Operator-owned Search Head roll, indexer replacement, network variants, controller restart, and soak remain open. See [SHC107PersistentClientQualificationExecPlan.md](SHC107PersistentClientQualificationExecPlan.md). |

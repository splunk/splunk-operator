...
| SHC-98 | Test whether stable per-ordinal indexer search addresses reduce the SHC-85 distributed-peer convergence window | Operator `5faeeb0e7`; Splunk Ansible `de208e179`; Docker-Splunk `f611b74` on `codex/shc-98-stable-indexer-search-address` | OPS-011, STS-004, OBS-001, OBS-003, OBS-005 | In progress. Live and source inspection established that Search Heads currently receive replaceable Pod-IP peer addresses when indexers omit Splunk's supported `register_search_address`; every retained indexer already has a stable StatefulSet/headless-Service FQDN. The candidate makes Ansible support explicit/`auto` pre-start registration without a restart, pins Docker-Splunk to the exact dependency, and makes the Operator choose `auto` only for clustered indexers while preserving customer `extraEnv` override. Focused tests and Operator `make build` pass; Linux source gates, immutable images, EKS rollout, per-Search-Head peer convergence, and workload comparison remain required. This is a hypothesis under test and does not remove the Splunk partial-result requirement. See [SHC98StableIndexerSearchAddressExecPlan.md](SHC98StableIndexerSearchAddressExecPlan.md). |

...
| SHC-98 | Test whether stable per-ordinal indexer search addresses reduce the SHC-85 distributed-peer convergence window | Operator `2c607d6e2`; Splunk Ansible `8a3ac3b4`; Docker-Splunk `cfd4fdee` on `codex/shc-98-stable-indexer-search-address` | OPS-011, STS-004, OBS-001, OBS-003, OBS-005 | In progress. Live and source inspection established that Search Heads currently receive replaceable Pod-IP peer addresses when indexers omit Splunk's supported `register_search_address`; every retained indexer already has a stable StatefulSet/headless-Service FQDN. The candidate makes Ansible support explicit/`auto` pre-start registration without a restart, reserves `absent` for verified controlled rollback while undefined/null/empty remain unmanaged, pins Docker-Splunk to the exact dependency, and makes the Operator choose `auto` only for clustered indexers when the existing Pod and Indexer lifecycle Alpha gates are enabled while preserving customer `extraEnv` override. The complete local Ansible SHC Make gate, Docker dependency gates, ten repeated Operator feature-gate/override tests, and Operator `make build` pass. A disposable composition with the independent SHC-99 process matcher passed all 43 local suites and 192 controller specs with zero failures. Linux full source gates, immutable images, EKS rollout, per-Search-Head peer convergence, and workload comparison remain required. This is a hypothesis under test and does not remove the Splunk partial-result requirement. See [SHC98StableIndexerSearchAddressExecPlan.md](SHC98StableIndexerSearchAddressExecPlan.md). |

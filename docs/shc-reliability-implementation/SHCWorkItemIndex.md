...
| SHC-104 | Make the Docker-Splunk aggregate test bootstrap reproducible on current supported Linux builders | Candidate `0604eeb` on `codex/shc-104-docker-test-bootstrap`; native-Linux gate pending | OBS-005 | In progress. The candidate creates a repository-local `.test-venv`, locks the Python 3.10 dependency graph and PyYAML build toolchain, retains Docker Compose 1.29.2, routes all broader pytest commands through that environment, and adds five bootstrap regressions. Clean and repeated local Python 3.10 runs pass; all 91 broader image tests collect, Compose configuration validates, and the existing 15 shutdown, four exact-Ansible-ref, and one base-image-security tests retain their counts. This remains test-infrastructure work and does not change runtime source. Acceptance still requires the same clean and repeated gate on native Linux. See [SHC104DockerSplunkTestBootstrapExecPlan.md](SHC104DockerSplunkTestBootstrapExecPlan.md). |

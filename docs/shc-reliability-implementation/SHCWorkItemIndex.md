...
| SHC-108 | Classify expected SHC member/captain observation transients without hiding persistent, quorum-threatening, or outside-lifecycle failures | Requirements only on `codex/shc-108-transient-observation-severity` | OBS-001, OBS-002, OBS-003, OBS-005, LFC-009, LFC-010 | Planned. SHC-107 active-captain deletion completed 600 events exactly but produced 23 captain-election 503 and 16 member-info connection-refusal ERROR records during the known termination/election window. Classification must use durable target/stage, Pod deletion and endpoint state, elapsed budget, serving floor, quorum, and unrelated-member health. Expected intervals remain observable and deduplicated; timeout, contradiction, capacity loss, or failures outside an owned operation remain errors. See [SHC108TransientObservationSeverityExecPlan.md](SHC108TransientObservationSeverityExecPlan.md). |

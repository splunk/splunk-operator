...
| SHC-104 | Make the Docker-Splunk aggregate test bootstrap reproducible on current supported Linux builders | Open; reproduced on canonical Docker-Splunk source `6ee266c1` | OBS-005 | Open test-infrastructure item. The 15 shutdown, four exact-Ansible-ref, and one base-image-security Make tests all pass and the repository remains clean. The aggregate `make test_setup` subsequently fails while installing its broader legacy Python dependencies: pip 26.2/Python 3.10 selects PyYAML 5.4.1 through Docker Compose 1.29.2, and the isolated source build fails because the modern build backend lacks `cython_sources`. This is not a runtime qualification failure. Acceptance requires a repository-owned isolated and locked bootstrap that passes twice from a clean Linux checkout without changing user site-packages. See [SHC104DockerSplunkTestBootstrapExecPlan.md](SHC104DockerSplunkTestBootstrapExecPlan.md). |

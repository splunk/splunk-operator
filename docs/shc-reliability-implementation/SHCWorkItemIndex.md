...
| SHC-107 | Qualify long-lived HEC and distributed-search client connections across Kubernetes Service backend replacement | Test-only source `9e9cbc819` on `codex/shc-107-persistent-client-qualification` | OPS-006, OPS-011, OBS-001, OBS-003, OBS-005 | Harness source-qualified; live qualification pending. The existing monitors start a new client process per request and cannot prove behavior of an established TCP/TLS flow. SHC-107 reuses HEC and Search Head HTTPS connections, records the Search Head identity and connection generation, distinguishes first-attempt transport failures from bounded reconnect recovery and logical failure, and preserves exact final completeness. Seven tests passed 100 repetitions with `make fmt vet` and Kubernetes manifest validation. Stable smoke, Search Head/indexer roll, network variants, controller restart, and soak remain open. See [SHC107PersistentClientQualificationExecPlan.md](SHC107PersistentClientQualificationExecPlan.md). |

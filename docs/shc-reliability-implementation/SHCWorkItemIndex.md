...
| SHC-107 | Qualify long-lived HEC and distributed-search client connections across Kubernetes Service backend replacement | Test-only source `f3ec88026` on `codex/shc-107-persistent-client-qualification` | HLT-014, OPS-006, OPS-011, OBS-001, OBS-003, OBS-005 | Harness and stable reuse are qualified; disruptive qualification remains open. The existing monitors start a new client process per request and cannot prove behavior of an established TCP/TLS flow. SHC-107 reuses HEC and Search Head HTTPS connections, records the Search Head identity and connection generation, distinguishes first-attempt transport failures from bounded reconnect recovery and logical failure, and preserves exact final completeness. Seven tests passed 100 repetitions with `make fmt vet` and Kubernetes manifest validation. On the accepted EKS deployment, 12 HEC writes reused one connection, 25 Search Head identity/search requests reused one connection, both returned HTTP/1.1 `Keep-Alive`, and all requests and final unique events completed exactly. Diagnostic runs also proved current Splunkd forces connection close for absent/default Python and official Python SDK user-agent identities; a neutral capable-client identity permits reuse. Search Head/indexer roll, network variants, controller restart, and soak remain open. See [SHC107PersistentClientQualificationExecPlan.md](SHC107PersistentClientQualificationExecPlan.md). |

...
| SHC-98 | Test whether stable per-ordinal indexer search addresses reduce the SHC-85 distributed-peer convergence window | Operator `2c607d6e2`; monitor `78ff404c7`; workload `aa9a566aa`; Splunk Ansible `56b93b9de`; Docker-Splunk `58fd9736` on `codex/shc-98-stable-indexer-search-address` | OPS-011, STS-004, OBS-001, OBS-003, OBS-005 | In progress. Live and source inspection established that Search Heads currently receive replaceable Pod-IP peer addresses when indexers omit Splunk's supported `register_search_address`; every retained indexer already has a stable StatefulSet/headless-Service FQDN. The candidate makes Ansible support customer-safe explicit/`auto` pre-start registration without a restart: `auto` uses a non-empty container hostname or the system FQDN while preserving an existing unowned effective value, explicit or newly adopted values record the last managed value, automation acts as owner only while the effective value matches that record, and a different customer value is preserved while stale ownership is relinquished. `absent` removes only a still-owned value; undefined/null/empty remain unmanaged. Six executable Ansible scenarios prove adoption, idempotence, unmanaged preservation, owned rollback, explicit override, and customer takeover after prior ownership. Docker-Splunk pins the exact dependency, and the Operator chooses `auto` only for clustered indexers when the existing Pod and Indexer lifecycle Alpha gates are enabled while preserving customer `extraEnv` override. The complete local Ansible SHC Make gate, Docker dependency gates, ten repeated Operator feature-gate/override tests, and Operator `make build` pass. A disposable composition with the independent SHC-99 process matcher passed all 43 local suites and 192 controller specs with zero failures. A live read-only snapshot validates the per-Search-Head and Cluster Manager evidence path and exposed that `OnDelete` completion requires all Pods on `updateRevision`, not status revision equality. The separate API-independent workload passed live server-side dry-run. Linux full source gates, immutable images, EKS rollout, customer-collision/rollback negatives, per-Search-Head peer convergence, and workload comparison remain required. This is a hypothesis under test and does not remove the Splunk partial-result requirement. See [SHC98StableIndexerSearchAddressExecPlan.md](SHC98StableIndexerSearchAddressExecPlan.md). |

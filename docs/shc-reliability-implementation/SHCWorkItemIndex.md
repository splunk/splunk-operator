...
| SHC-111 | Qualify explicit HTTP/HTTPS endpoint protocols for the persistent HEC/search client without assuming ingress or service mesh | Test source `de6f5f8e3` on `codex/shc-111-protocol-qualification` | HLT-014, OPS-011, OBS-001, OBS-003, OBS-005 | Source and direct-Service HTTP qualification are complete; HTTPS restoration remains open. HTTPS remains the default, endpoint scheme and port are explicit per path, unsupported schemes fail closed, and 19 tests plus manifest validation passed 100 repetitions. A stable HTTP run completed 100/100 on one connection. A same-protocol `3 -> 2 -> 1 -> 0` roll retained at least three Ready Pods and endpoints, preserved every PVC, changed all four Pod UIDs/IPs, and recorded zero restarts. Its client recovered one HEC 503 and delivered 2,400/2,400 exactly with zero request or identity failures, but three successful searches regressed by up to 933 events and peak pending was 935. The monitor rejected ordinal 2 selection before ordinal 3 convergence; exact four-peer convergence followed lifecycle completion by 1,577 seconds. This reproduces the HTTPS behavior and rules out HEC TLS as the sequencing cause. The mixed HTTPS-to-HTTP setup transition remains setup-only evidence. The cluster has no IngressClass or detected mesh control plane/sidecar, so ingress and mesh variants remain unqualified rather than assumed. See [SHC111ProtocolQualificationExecPlan.md](SHC111ProtocolQualificationExecPlan.md). |

...
| SHC-107 | Qualify long-lived HEC and distributed-search client connections across Kubernetes Service backend replacement | Stable/captain source `f3ec88026`; response-aware source `d57db8d7a` on `codex/shc-107-persistent-client-qualification` | HLT-014, OPS-006, OPS-011, OBS-001, OBS-003, OBS-005 | Stable reuse, unplanned active-captain replacement, and one selected-indexer replacement are qualified for bounded claims. Stable EKS carried 12 HEC writes on one connection and 25 Search Head requests on one connection. Deleting the selected captain caused one failed first attempt and one recovered request, then completed 600 events exactly. Indexer negative testing proved EndpointSlice withdrawal cannot move an established HEC flow: Splunk returned code 23 / HTTP 503 with `Connection: Keep-Alive`, and transport-only recovery lost 28 rejected submissions. Response-aware source passed 12 focused tests and 100 Make repetitions; closing and retrying that explicit rejection once through the Service recovered one request and completed all 600 submissions exactly. Two HTTP-successful searches returned lower counts during recovery, so OPS-011 immediate completeness remains open. Operator-owned rolls, network variants, controller restart, and soak remain open. See [SHC107PersistentClientQualificationExecPlan.md](SHC107PersistentClientQualificationExecPlan.md). |

...
| SHC-118 | Require continuous Kubernetes endpoint withdrawal before Search Head detention | Planned source branch `codex/shc-118-search-head-endpoint-withdrawal` from exact SHC-116 production source | LFC-001, LFC-003, LFC-008, HLT-014, OBS-001, OBS-002, OBS-003, OBS-005 | Registered; implementation and qualification remain open. The Search Head path already fails closed on serving-gate, Pod Ready, exact UID, client Service, and EndpointSlice readiness, but it can request detention immediately once withdrawal is observed. SHC-118 will persist an effective deadline and monotonic observation/invalidation state before detention, while retaining the existing drain, captain, replacement, rejoin, cancellation, and rollback gates. This reduces new routing but does not claim to terminate established client connections. See [SHC118SearchHeadEndpointWithdrawalExecPlan.md](SHC118SearchHeadEndpointWithdrawalExecPlan.md). |

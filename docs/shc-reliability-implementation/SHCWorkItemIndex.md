...
| SHC-99 | Match only the exact Splunkd daemon in process-based probes | Functional source `184061106`; documentation tip `4b6c6480b` on `codex/shc-99-exact-splunkd-process-match` | HLT-009, OBS-005 | Source-qualified separately. A broad process-name match produced a false positive when an unrelated command contained both `splunkd` and `autostart`. The exact matcher passed focused tests and the serialized native-Linux gate with all 43 suites and 192 controller specs. It remains isolated from SHC-98 and requires immutable live no-regression qualification before integration. |
| SHC-100 | Keep stable indexer search identity explicit and reject ordinal advancement evidence when the prior peer identity has not converged on every Search Head | Operator safety `ca8c5eab6`; monitor guard `cc63afe81`; rollback mode `bbeaa5644`; evidence preservation `91c39b89b`; stale-target isolation `c952d242a` on `codex/shc-100-safe-stable-indexer-address-opt-in` | OPS-011, STS-004, OBS-001, OBS-003, OBS-005 | In progress. The Operator no longer changes persistent peer identity merely because Pod and Indexer lifecycle gates are enabled; an explicit customer `extraEnv` value remains preserved for coordinated qualification. The read-only monitor records a violation when a later target appears before the completed peer GUID has exactly one `Up` expected identity on every Search Head, preserves the remaining evidence, and fails final qualification even after eventual convergence. The controlled EKS rollback completed `3 -> 2 -> 1 -> 0` with one maximum unavailable Pod, at least three Ready endpoints, preserved PVCs, zero restarts, and exact final four-peer agreement, then failed as designed because ordinal 2 was selected before ordinal 3 converged on every Search Head. The 3,600-event workload completed exactly with zero request failures while still exposing 221 materially incomplete successful samples. Focused Go tests, formatting, Bash syntax, ShellCheck, live snapshot smoke tests for both modes, exact `jq` predicates, and a disposable composition with SHC-99 that passed all 43 suites and 192 controller specs are recorded. Exact native-Linux safety image qualification, accepted-image restoration, fresh-cluster explicit-address formation and replacement, customer takeover, and final integration remain open. |

...
| SHC-108 | Classify and deduplicate SHC member/captain observation transients without hiding unexpected, persistent, or quorum-threatening failures | Requirements only on `codex/shc-108-transient-observation-severity` | OBS-001, OBS-002, OBS-003, OBS-005, LFC-009, LFC-010 | Planned. SHC-107 unplanned active-captain deletion completed 600 events exactly but produced 23 captain-election 503 and 16 member-info connection-refusal ERROR records during one replacement window. Current source already classifies an owned planned target member as temporarily unavailable; the missing boundary is captain observation plus repeated polls after an unplanned deletion. An unplanned disruption retains one clear error transition, then identical observations may be coalesced only while Kubernetes identifies the same lost Pod, recovery is bounded, and capacity/quorum are safe. Timeout, contradiction, unrelated-member failure, or capacity loss escalates. See [SHC108TransientObservationSeverityExecPlan.md](SHC108TransientObservationSeverityExecPlan.md). |
| SHC-109 | Make Splunk HEC close an established connection when detention/shutdown explicitly rejects new work | Requirements only on `codex/shc-109-hec-shutdown-connection`; Splunkd implementation deferred | HLT-003, HLT-007, HLT-014, OPS-011, OBS-003, OBS-005 | Planned Splunk Enterprise work. Source and EKS evidence show code 23 / HTTP 503 `Server is shutting down` is returned with `Connection: Keep-Alive`; Kubernetes EndpointSlice withdrawal cannot migrate that existing flow. A bounded client mitigation recovered one explicit rejection and delivered 600/600 events, but ambiguous transport failures still require HEC ACK/idempotency. Splunkd should gracefully close the connection after the rejection while preserving accepted in-flight and ACK semantics. Operator readiness and Docker-Splunk preStop cannot close Splunkd-owned sockets. See [SHC109HECShutdownConnectionExecPlan.md](SHC109HECShutdownConnectionExecPlan.md). |
| SHC-110 | Make Splunk close an established Search Head connection after detention explicitly rejects new search creation | Requirements only on `codex/shc-110-search-detention-connection`; test client `3e9f47751`; Splunkd implementation deferred | HLT-003, HLT-007, HLT-014, OPS-006, OBS-003, OBS-005 | Planned Splunk Enterprise work; response-aware EKS qualification is in progress. During an Operator-owned roll, the selected Search Head remained reachable on its established connection after Service withdrawal; new export searches failed until a later transport break moved the client. Splunk source intentionally returns HTTP 405 for new search creation in detention and directs the user to another member, but the connection stays reusable. The server should close it gracefully after the refusal. The bounded client closes/retries that exact endpoint/status once and keeps a second rejection as failure. See [SHC110SearchDetentionConnectionExecPlan.md](SHC110SearchDetentionConnectionExecPlan.md). |
| SHC-111 | Qualify explicit HTTP/HTTPS endpoint protocols for the persistent HEC/search client without assuming ingress or service mesh | Test source `de6f5f8e3` on `codex/shc-111-protocol-qualification` | HLT-014, OPS-011, OBS-001, OBS-003, OBS-005 | Source, direct-Service HTTP qualification, and dependency-safe HTTPS restoration are complete. HTTPS remains the default, endpoint scheme and port are explicit per path, unsupported schemes fail closed, and 19 tests plus manifest validation passed 100 repetitions. A stable HTTP run completed 100/100 on one connection. A same-protocol HTTP `3 -> 2 -> 1 -> 0` roll retained at least three Ready Pods and endpoints, preserved every PVC, changed all four Pod UIDs/IPs, and recorded zero restarts. Its client recovered one HEC 503 and delivered 2,400/2,400 exactly with zero request or identity failures, but three successful searches regressed by up to 933 events and peak pending was 935. The monitor rejected ordinal 2 selection before ordinal 3 convergence; exact four-peer convergence followed lifecycle completion by 1,577 seconds. HTTPS restoration repeated reverse-order replacement with the same availability, identity, PVC, and restart results; all four effective configurations reported `enableSSL=1`, the Service and four headless endpoints returned HTTPS health 200, and HTTP was reset. The monitor again rejected premature ordinal-2 selection; final exact peer convergence followed lifecycle completion by 2,026 seconds and remained exact for 13 observations. This rules out HEC TLS as the sequencing cause. The mixed protocol setup/restoration transitions are not workload-availability verdicts. The cluster has no IngressClass or detected mesh control plane/sidecar, so ingress and mesh variants remain unqualified rather than assumed. See [SHC111ProtocolQualificationExecPlan.md](SHC111ProtocolQualificationExecPlan.md). |
...
| SHC-116 | Require continuous Kubernetes endpoint withdrawal before Operator-owned indexer decommission | Exact source `96c83dcad` on `codex/shc-116-indexer-endpoint-withdrawal`, stacked on SHC-115 | OPS-011, STS-004, HLT-014, OBS-001, OBS-002, OBS-003, OBS-005 | Complete for the bounded EKS request-success and eventual-delivery gate. The accepted-image diagnostic aligned its only HEC failure with the decommission request while the target still appeared routable through the client Service. SHC-116 requires exact target Pod readiness withdrawal plus no routable EndpointSlice target, persists the effective deadline and UID, invalidates an interrupted quiet interval monotonically, and gates both normal and recovery decommission. Exact source passed 43 native-Linux suites, 192/192 specs, 78.7 percent coverage, generation, build/vet, focused race checks, chart lint, all 150 Helm tests, and produced immutable OCI index `sha256:f16a128454cbe60f2d6230c452c69efca6dafe6526b8f2589bdc27b3203a3a1f`. EKS completed controller replacement during the delay, target order `3,2,1,0`, 647 monitor snapshots, 60 final-state observations, stable claims, zero restarts, zero failures across 10,800 HEC and search requests, exact final uniqueness, and 25 verified artifact hashes. Nineteen HTTP-successful count regressions occurred during the planned roll, so Splunk's separate complete-intermediate-result requirement remains open. See [SHC116IndexerEndpointWithdrawalExecPlan.md](SHC116IndexerEndpointWithdrawalExecPlan.md). |
| SHC-117 | Keep the workload and monitor active through a complete exact-peer-gated four-indexer roll | Exact test source `cd522e119` on `codex/shc-117-long-indexer-roll-qualification`, stacked on SHC-116 | OPS-011, HLT-014, OBS-003, OBS-005 | Complete. The observed per-ordinal stale-peer cleanup can make a complete roll exceed the former one-hour workload and two-hour monitor. SHC-117 changes only the qualification fixture: 10,800 one-second samples, a 10,800-second monitor timeout, and a 14,400-second Job deadline. The extended EKS run stayed active through all four replacements and stabilization, finished with zero HEC/search-request failures, exact final count/min/max/distinct `10800/1/10800/10800`, monitor and outer-runner exit codes zero, and all 25 hashes verified. It independently reports 19 intermediate count regressions and maximum pending 1,335 during the planned roll. The production Operator and runtime images are unchanged. See [SHC117LongIndexerRollQualificationExecPlan.md](SHC117LongIndexerRollQualificationExecPlan.md). |
| SHC-118 | Require continuous Kubernetes endpoint withdrawal before Search Head detention | Exact source `8152fc042` on `codex/shc-118-search-head-endpoint-withdrawal`, stacked on SHC-116; Operator OCI index `sha256:bc733990967abade9419be4caa85d68040355c959d86410a93bd8765830eed9f`; harness `7363f71a9` | LFC-001, LFC-003, LFC-008, HLT-014, OBS-001, OBS-002, OBS-003, OBS-005 | Complete for the bounded endpoint-withdrawal, request-availability, and eventual-delivery scope. The controller-restart campaign passed target order `2,1,0`, exact 120-second deadlines, manager replacement inside ordinal 2's interval, dynamic captain transfer, minimum two endpoints, maximum one unready Pod, three replacement UIDs with stable claims, zero invalidations/restarts/request failures, and 12 stable samples. Its Job completed 3,600 submissions with zero HEC/search failures, zero count regressions, maximum pending 2, exact final uniqueness, zero runner/wait/finalizer exit codes, and 37 verified hashes. The omitted-field campaign independently passed exact 30-second intervals for all three ordinals, dynamic captain transfer, the same endpoint/unready/restart/claim bounds, zero invalidations/request failures, and 12 stable samples. Its Job completed 3,600 submissions with zero HEC/search failures, one transient count regression, maximum pending 3, exact final uniqueness, zero runner/wait/finalizer exit codes, and 30 verified hashes. The successful-search regression self-healed on the next sample and keeps the separate Splunk intermediate-result semantics requirement open. The missing field is resolved by the Operator, not defaulted by Kubernetes. Event-series counts are at-least-once signals; status/sample operation IDs prove exact cardinality. Generation, manifests, focused tests, all 43 native suites (192/192 specs, 78.8 percent coverage), `make build`, chart lint, all 150 Helm tests, full v4 API/metrics race checks, and 20 changed-path enterprise race repetitions passed. Existing drain, captain, replacement, rejoin, cancellation, and rollback gates remain in force. This reduces new routing but does not claim to terminate established client connections. See [SHC118SearchHeadEndpointWithdrawalExecPlan.md](SHC118SearchHeadEndpointWithdrawalExecPlan.md). |

...
parent: Reference
nav_order: 6
permalink: /docs/SplunkGeneralTermsMigration.html
---
...
To learn more about the required value for this variable, please see the [README](../index.md#splunk-general-terms-acceptance) or Splunk Enterprise 10.x image’s README.
...
1. Pass the `SPLUNK_GENERAL_TERMS` parameter with the [required value](../index.md#splunk-general-terms-acceptance) to the `make deploy` command.
```bash
make deploy IMG=docker.io/splunk/splunk-operator:<tag name> SPLUNK_GENERAL_TERMS="<required value>"
```
2. Update the value in the Splunk Operator installation file from the [release](https://github.com/splunk/splunk-operator/releases/latest) on GitHub with the [required value](../index.md#splunk-general-terms-acceptance).
```yaml
...
        env:
...
...
```
3. Set the [required value](../index.md#splunk-general-terms-acceptance) in a `helm install` command.
...
```
4. Edit the splunk-operator-controller-manager deployment after it is deployed to include the [required value](../index.md#splunk-general-terms-acceptance).
```bash

...
For more information about the custom resources that you can use with the Splunk Operator, refer to the [Custom Resource Guide](../operate/CustomResources.md).
...
See [Configuring Ingress](../deploy/Ingress.md) for guidance on making your Splunk Enterprise clusters accessible from outside of Kubernetes.
...
has been disabled. Please review [`PasswordManagement.md`](../operate/PasswordManagement.md)
...
With the Splunk Operator 2.0 release, new App Framework is available to centrally store and deploy apps. See [AppFramework](../operate/AppFramework.md) for information and examples.
...
#### Approach 1
- Setup the desired plain-text [`pass4Symmkey`](../operate/PasswordManagement.md#pass4Symmkey) in the global secret object(Note: The `pass4Symmkey` would be stored in a base64 encoded format). For details see [updating global kubernetes secret object](#updating-global-kubernetes-secret-object).
...
- Retrieve the plain-text `pass4SymmKey` in the `[general]` section of your LM's `server.conf` file.
  ...
  ```
- Setup the above decrypted plain-text [`pass4Symmkey`](../operate/PasswordManagement.md#pass4Symmkey) in the global secret object(Note: The `pass4Symmkey` would be stored in a base64 encoded format). For details see [updating global kubernetes secret object](#updating-global-kubernetes-secret-object)
...
#### Approach 1
- Setup the desired plain-text [`IDXC pass4Symmkey`](../operate/PasswordManagement.md#idxc-pass4Symmkey) in the global secret object(Note: The `IDXC pass4Symmkey` would be stored in a base64 encoded format). For details see [updating global kubernetes secret object](#updating-global-kubernetes-secret-object).
...
- Retrieve the plain-text `IDXC pass4SymmKey` in the `[clustering]` section of your cluster manager's `server.conf` file.
  ...
  $SPLUNK_HOME/bin/splunk show-decrypted --value '$7$Sw0A+wvJdTztMcA2Ge7u435XmpTzPqyaq49kUZqn0yfAgwFpwrArM2JjWJ3mUyf/FyHAnCZkE/U='
  ```
- Setup the above decrypted plain-text [`IDXC pass4Symmkey`](../operate/PasswordManagement.md#idxc-pass4Symmkey) in the global secret object(Note: The `IDXC pass4Symmkey` would be stored in a base64 encoded format). For details see [updating global kubernetes secret object](#updating-global-kubernetes-secret-object)
...
2. Gather the password values for the secret tokens you want to configure. To see all available secret tokens defined for the global kubernetes secret object, review [password management](../operate/PasswordManagement.md#splunk-secret-tokens-in-the-global-secret-object)
...
2. Obtain the key name for the secret token you are populating. The list of tokens is available in [password management](../operate/PasswordManagement.md#splunk-secret-tokens-in-the-global-secret-object). 
...
When a CR is created for the first time in a specific namespace, and the probe ConfigMap for that namespace is not yet created, the Splunk Operator will automatically create the `splunk-<CR namespace>-probe-configmap` ConfigMap with the default scripts from the tools/k8_probes/ directory. The Operator can update an unchanged set of generated defaults when a later Operator image includes revised scripts. Once you edit any data in the ConfigMap, the Operator preserves the complete ConfigMap data and does not overwrite your customization. All Splunk CRs in that namespace continue to share the customized scripts.

Probe ConfigMaps created by an earlier Operator version might not contain the internal content marker that distinguishes unchanged generated defaults from customer-managed data. The Operator preserves every ConfigMap without that marker because it cannot safely determine who created or changed it. If you deliberately want to replace an unmodified legacy ConfigMap with the current generated defaults, first back it up, delete it, and allow the Operator to recreate it during reconciliation. Do not delete a ConfigMap that contains required custom scripts.

### Create the ConfigMap before deploying CRs

Before deploying any Custom Resources to a specific namespace, you can manually create the ConfigMap with your unique scripts as the data. All three scripts must be included in the ConfigMap whether you are using a custom script for one or all of the probes. The scripts must be named exactly `livenessProbe.sh`, `readinessProbe.sh`, and `startupProbe.sh`. If you only require a custom script for one or two probes, please make sure to add the default script for the other probes to your ConfigMap. **The Operator will never overwrite the data in a pre-created `splunk-<CR namespace>-probe-configmap` ConfigMap.** Only a user can manually update the scripts if needed.

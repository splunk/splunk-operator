...
- [Integration Test Debugging Flow](#integration-test-debugging-flow)
- [Helpful AI Prompts](#helpful-ai-prompts)
  - [Debug a Unit or Integration Test](#debug-a-unit-or-integration-test)
  - [Debug a Bug or Customer Issue](#debug-a-bug-or-customer-issue)
- [Links to Troubleshooting Pages](#links-to-troubleshooting-pages)

---

## Integration Test Debugging Flow

Follow the steps in this section to run and debug integration tests on your local machine. Use a disposable cluster, such as a Kraken cluster, so canceled or failed runs do not disturb shared environments.

1. Prepare a disposable integration-test cluster.
   - See [Set Up SOK with Kraken](./SetupWithKraken.md) for steps to deploy a Kubernetes environment and the Splunk Operator.
...
2. Configure the local repo for test runs.
   ...
   - If you are using a Kraken setup, include the KUBECONFIG in the `test.env` file:

     ```bash
     KUBECONFIG=/path/to/.kube/kraken/<DEPLOYMENT_ID>.yaml
     ```
...
4. Isolate one test.
   ...
   - If your runner cannot pass Ginkgo focus flags reliably, or you are using an IDE, temporarily add `F` before `Context` or `It` to focus the case, or use `XContext` or `XIt` to skip cases that should not run.
   - **Remove any temporary focus or skip markers before committing**.

5. Run or debug the suite, and capture durable logs when needed.
   ...
   - For CLI runs that need a durable log, capture the equivalent `go test` command from your runner output or run a suite directly:

     ```bash
     set -a
     source ./test.env
     set +a
     go test -timeout 1200m -run "^TestBasic$" github.com/splunk/splunk-operator/test/appframework_aws/s1 -v > /tmp/sok-test-output.log 2>&1 &
     ```
   ...
   - Add targeted debug output when breakpoints are not enough.
...
6. Inspect the cluster status when a breakpoint is hit.
   - Be cautious not to influence its state. Only look at what already exists without changing it.
...
7. Stop a stuck run deliberately.
...
8. Clean the test cluster before the next run.
...
To connect to GitLab, Jira, and other internal systems, set up [Ghost](http://go/ai-handbook), and make sure to authenticate before running any of the prompts.

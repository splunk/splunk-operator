...
// the NoahCluster object against the actual cluster state, and then
...
func (r *NoahClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	// your logic here
	reconcileCounters.With(getPrometheusLabels(req, "NoahCluster")).Inc()
	defer recordInstrumentionData(time.Now(), req, "controller", "NoahCluster")

	reqLogger := log.FromContext(ctx)
	reqLogger = reqLogger.WithValues("NoahCluster", req.NamespacedName)
	...
	if annotations != nil {
		if _, ok := annotations[enterpriseApi.NoahClusterPausedAnnotation]; ok {
	...
	result, err := ApplyNoahCluster(ctx, r.Client, instance)
...
// ApplyNoahCluster adding to handle unit test case
var ApplyNoahCluster = func(ctx context.Context, client client.Client, instance *enterpriseApi.NoahCluster) (reconcile.Result, error) {
	return enterprise.ApplyNoahCluster(ctx, client, instance)
}
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&source.Kind{Type: &appsv1.Deployment{}},
			&handler.EnqueueRequestForOwner{
				IsController: false,
				OwnerType:    &enterpriseApi.NoahCluster{},
			}).

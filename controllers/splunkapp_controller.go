...
//+kubebuilder:rbac:groups=enterprise.splunk.com,resources=spalunkapps,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=enterprise.splunk.com,resources=spalunkapps/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=enterprise.splunk.com,resources=spalunkapps/finalizers,verbs=update
...
func (r *SplunkAppReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	instance := &enterpriseApi.SplunkApp{}
	...
	if err != nil {
		...
		return ctrl.Result{}, errors.Wrap(err, "could not load splunkapp data")
	}

	// If the reconciliation is paused, requeue
	annotations := instance.GetAnnotations()
	if annotations != nil {
		if _, ok := annotations[enterpriseApi.SplunkAppPausedAnnotation]; ok {
	...
	result, err := ApplySplunkApp(ctx, r.Client, instance)
...
var ApplySplunkApp = func(ctx context.Context, client client.Client, instance *enterpriseApi.SplunkApp) (reconcile.Result, error) {
	return enterprise.ApplySplunkApp(ctx, client, instance)
}

// SetupWithManager sets up the controller with the Manager.
func (r *SplunkAppReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&enterpriseApi.SplunkApp{}).
		...
		Watches(&source.Kind{Type: &appsv1.StatefulSet{}},
			&handler.EnqueueRequestForOwner{
				IsController: false,
				OwnerType:    &enterpriseApi.SplunkApp{},
			}).

...
SHC118_MONITOR ?= test/fixtures/shc-reliability/shc118_endpoint_withdrawal_monitor.sh
...
shc83-license-secret: ## Create or update the SHC-83 LicenseManager license Secret.
	@test -n "$(SHC83_LICENSE_FILE)" || \
		{ echo "SHC83_LICENSE_FILE must point to a valid remote-manager-capable Splunk license"; exit 1; }
	@test -f "$(SHC83_LICENSE_FILE)" || \
...
shc85-license-secret: ## Create or update the SHC-85 LicenseManager license Secret.
	@test -n "$(SHC85_LICENSE_FILE)" || \
		{ echo "SHC85_LICENSE_FILE must point to a valid remote-manager-capable Splunk license"; exit 1; }
	@test -f "$(SHC85_LICENSE_FILE)" || \
		{ echo "license file not found: $(SHC85_LICENSE_FILE)"; exit 1; }
...
shc118-monitor-check: ## Validate the SHC-118 endpoint-withdrawal qualification monitor.
	bash -n "$(SHC118_MONITOR)"
	shellcheck "$(SHC118_MONITOR)"

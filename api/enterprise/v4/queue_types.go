...
// +kubebuilder:validation:XValidation:rule="(has(self.sqs.endpoint) == has(oldSelf.sqs.endpoint)) && (!has(self.sqs.endpoint) || self.sqs.endpoint == oldSelf.sqs.endpoint)",message="sqs.endpoint is immutable once created"

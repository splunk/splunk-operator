...
if [ "${use_existing_cluster}" = "true" ]; then
  if [ -z "${existing_cluster_name}" ]; then
    echo "Existing-cluster mode requires PIPELINE_INT_EXISTING_CLUSTER_NAME or JOB_EXISTING_CLUSTER_NAME" >&2
    exit 1
  fi
...
export TEST_SQS_QUEUE="$(first_nonempty "${PIPELINE_TEST_SQS_QUEUE:-}" "${TEST_SQS_QUEUE:-}" "")"
export TEST_SQS_DLQ="$(first_nonempty "${PIPELINE_TEST_SQS_DLQ:-}" "${TEST_SQS_DLQ:-}" "")"
export TEST_S3_BUCKET_PATH="$(first_nonempty "${PIPELINE_TEST_S3_BUCKET_PATH:-}" "${TEST_S3_BUCKET_PATH:-}" "")"
export TEST_AWS_REGION="$(first_nonempty "${PIPELINE_TEST_AWS_REGION:-}" "${TEST_AWS_REGION:-}" "")"
export TEST_SQS_ENDPOINT="$(first_nonempty "${PIPELINE_TEST_SQS_ENDPOINT:-}" "${TEST_SQS_ENDPOINT:-}" "")"
export TEST_S3_ENDPOINT="$(first_nonempty "${PIPELINE_TEST_S3_ENDPOINT:-}" "${TEST_S3_ENDPOINT:-}" "")"
...
# creds-helper selects DOCKER_CONFIG. Configure all source registries first so
# the ECR login below is written to the same Docker config.

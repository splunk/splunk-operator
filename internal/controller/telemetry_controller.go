...
func (r *TelemetryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, err error) {
	...
	defer func() {
		if rec := recover(); rec != nil {
			...
			result = ctrl.Result{Requeue: true, RequeueAfter: telemetryRetryDelay}
			logger.ErrorContext(ctx, "recovered from panic in TelemetryReconciler.Reconcile; requeueing", "error", err, "periodSeconds", int(telemetryRetryDelay/time.Second))
		}
	}()

...
readonly PROGRAM_SOURCE="https://github.com/splunk/splunk-operator/tools"
readonly PROGRAM_NAME="$(basename "$0" .sh)"
readonly PROGRAM_DIR="/opt/log-collector"
readonly LOG_DIR="/var/log"
readonly COLLECT_DIR="/tmp/k8s-log-collector"
...
create_directories() {
  ...
  # the K8s is used, as it will have it.
  mkdir -p "${PROGRAM_DIR}"
...
cleanup() {
  ...
  if [[ "${COLLECT_DIR}" == "/tmp/k8s-log-collector" ]]; then
...
pack() {
  ...
  tar --create --verbose --gzip --file "${LOG_DIR}"/k8s_"${INSTANCE_ID}"_"${CURRENT_TIME}"_"${PROGRAM_VERSION}".tar.gz --directory="${COLLECT_DIR}" . > /dev/null 2>&1
...
finished() {
  cleanup
  echo -e "\n\tDone... your bundled logs are located in ${LOG_DIR}/k8s_${INSTANCE_ID}_${CURRENT_TIME}_${PROGRAM_VERSION}.tar.gz\n"
}
  ...
  for entry in ${COMMON_LOGS[*]}; do
    if [[ -e "/var/log/${entry}" ]]; then
        ...
        if [[ "${entry}" == "containers" ]]; then
          cp --force --dereference --recursive /var/log/containers/k8s-node* "${COLLECT_DIR}"/var_log/ 2>/dev/null
        ...
        if [[ "${entry}" == "pods" ]]; then
          cp --force --dereference --recursive /var/log/pods/kube-system_k8s-node* "${COLLECT_DIR}"/var_log/ 2>/dev/null
...
get_k8s_info() {
  ...
  case "${INIT_TYPE}" in
    systemd)
      ...
      ;;
    snap)
      timeout 75 snap logs kubelet-k8s -n all > "${COLLECT_DIR}"/kubelet/kubelet.log

      timeout 75 snap get kubelet-k8s > "${COLLECT_DIR}"/kubelet/kubelet-k8s_service.txt 2>&1
      ;;
    *)
      warning "The current operating system is not supported."
      ;;
...
get_ipamd_info() {
  ...
  cp /var/run/k8s-node/ipam.json "${COLLECT_DIR}"/ipamd/ipam.json
...
get_crd_info() {
  ...
  if [[ -n "${KUBECONFIG:-}" ]]; then
    ...
    command -v kubectl > /dev/null && kubectl get --kubeconfig="${KUBECONFIG}" crd clustermanager.enterprise.splunk.com >> "${COLLECT_DIR}"/splunk/crd.log
    ...
    command -v kubectl > /dev/null && kubectl get --kubeconfig="${KUBECONFIG}" crd licensemanager.enterprise.splunk.com >> "${COLLECT_DIR}"/splunk/crd.log
    ...
    command -v kubectl > /dev/null && kubectl get --kubeconfig="${KUBECONFIG}" clustermanager.enterprise.splunk.com >> "${COLLECT_DIR}"/splunk/crd.log
    ...
    command -v kubectl > /dev/null && kubectl get --kubeconfig="${KUBECONFIG}" licensemanager.enterprise.splunk.com >> "${COLLECT_DIR}"/splunk/crd.log

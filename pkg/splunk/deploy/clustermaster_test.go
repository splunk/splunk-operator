...
func TestReconcileClusterMaster(t *testing.T) {
	...
	reconcile := func(c *mockClient, cr interface{}) error {
		return ReconcileClusterMaster(c, cr.(*enterprisev1.ClusterMaster))
	}
	reconcileTester(t, "TestReconcileClusterMaster", &current, revised, createCalls, updateCalls, reconcile)
}

func TestApplyClusterMaster(t *testing.T) {
	funcCalls := []mockFuncCall{{metaName: "*v1alpha2.ClusterMaster-test-stack1"}}
	createCalls := map[string][]mockFuncCall{"Get": funcCalls, "Create": funcCalls}
	...
	reconcile := func(c *mockClient, cr interface{}) error {
		return applyClusterMaster(c, cr.(*enterprisev1.SplunkEnterprise))
	}
	reconcileTester(t, "TestApplyClusterMaster", &current, revised, createCalls, updateCalls, reconcile)
}

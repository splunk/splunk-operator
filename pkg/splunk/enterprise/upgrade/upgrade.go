...
func WaitForHistoricalSearches(ctx context.Context, client *splclient.SplunkClient, timeout time.Duration) error {
	...
	for {
		...
		case <-ticker.C:
			...
			// e.g. loop https://<splunk-server>:<port>/services/search/jobs?search=dispatchState%3Drunning%20OR%20dispatchState%3Dqueued%20OR%20dispatchState%3Dpending
			// use splclient package
			activeCount := 0

...
func TestUpgradePathValidation(t *testing.T) {
	...
	createPods(t , ctx, client, "indexer", fmt.Sprintf("splunk-%s-indexer-1",idx.Name), idx.Namespace, idx.Spec.Image)
	createPods(t , ctx, client, "indexer", fmt.Sprintf("splunk-%s-indexer-2",idx.Name), idx.Namespace, idx.Spec.Image)
	updateStatefulSetsInTest(t, ctx, client, 3, fmt.Sprintf("splunk-%s-indexer",idx.Name), idx.Namespace)

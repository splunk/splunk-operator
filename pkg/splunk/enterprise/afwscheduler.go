...
func (ppln *AppInstallPipeline) createAndAddPipelineWorker(ctx context.Context, phase enterpriseApi.AppPhaseType, appDeployInfo *enterpriseApi.AppDeploymentInfo,
	...
	logger := logging.FromContext(ctx).With("func", "createAndAddPipelineWorker")
	...
	logger.InfoContext(ctx, "Created new worker", "Pod name", worker.targetPodName, "App name", appDeployInfo.AppName, "digest", appDeployInfo.ObjectHash, "phase", appDeployInfo.PhaseInfo.Phase, "fan out", worker.fanOut)
...
func (ppln *AppInstallPipeline) addWorkersToPipelinePhase(ctx context.Context, phaseID enterpriseApi.AppPhaseType, workers ...*PipelineWorker) {
	logger := logging.FromContext(ctx).With("func", "addWorkersToPipelinePhase", "phase", phaseID)

	for _, worker := range workers {
		logger.InfoContext(ctx, "Adding worker", "name", worker.cr.GetName(), "namespace", worker.cr.GetNamespace(), "Pod name", worker.targetPodName, "App name", worker.appDeployInfo.AppName, "digest", worker.appDeployInfo.ObjectHash)
	}
...
func (ppln *AppInstallPipeline) deleteWorkerFromPipelinePhase(ctx context.Context, phaseID enterpriseApi.AppPhaseType, worker *PipelineWorker) bool {
	logger := logging.FromContext(ctx).With("func", "deleteWorkerFromPipelinePhase", "phase", phaseID)
	ppln.pplnPhases[phaseID].mutex.Lock()
	...
	for i, qWorker := range phaseQ {
		if worker == qWorker {
			...
			logger.InfoContext(ctx, "Deleted worker", "name", worker.cr.GetName(), "namespace", worker.cr.GetNamespace(), "Pod name", worker.targetPodName, "phase", phaseID, "App name", worker.appDeployInfo.AppName, "digest", worker.appDeployInfo.ObjectHash)
			return true
		}
	}
	return false
...
func (ppln *AppInstallPipeline) transitionWorkerPhase(ctx context.Context, worker *PipelineWorker, currentPhase, nextPhase enterpriseApi.AppPhaseType) {
	...
	logger := logging.FromContext(ctx).With("func", "transitionWorkerPhase", "App name", worker.appDeployInfo.AppName, "current Phase", currentPhase, "next phase", nextPhase)
	...
	if worker.fanOut {
		logger.InfoContext(ctx, "Fan-out transition")
		if currentPhase == enterpriseApi.PhaseDownload {
			...
			if len(appDeployInfo.AuxPhaseInfo) == 0 {
				...
				for podID := range appDeployInfo.AuxPhaseInfo {
					...
					logger.InfoContext(ctx, "Created a new fan-out pod copy worker", "pod name", worker.targetPodName)
				}
			} else {
				logger.InfoContext(ctx, "Installation was already in progress for replica members")
				...
				for podID := range appDeployInfo.AuxPhaseInfo {
					...
					if phaseInfo.Phase == enterpriseApi.PhaseInstall {
						...
						logger.ErrorContext(ctx, "invalid phase info detected", "phase", phaseInfo.Phase, "phase status", phaseInfo.Status)
					}
				}
			}
			...
			logger.ErrorContext(ctx, "Invalid phase detected")
		}

	} else {
		logger.InfoContext(ctx, "Simple transition")
	...
	logger.InfoContext(ctx, "Deleted worker", "phase", currentPhase)
...
func getPhaseInfoByPhaseType(ctx context.Context, worker *PipelineWorker, phaseType enterpriseApi.AppPhaseType) *enterpriseApi.PhaseInfo {
	if needToUseAuxPhaseInfo(worker, phaseType) {
		...
		if err != nil {
			logger := logging.FromContext(ctx).With("func", "getPhaseInfoByPhaseType")
			logger.ErrorContext(ctx, "unable to get the pod Id", "pod name", worker.targetPodName, "error", err)
			return nil
		}
...
func updatePplnWorkerPhaseInfo(ctx context.Context, appDeployInfo *enterpriseApi.AppDeploymentInfo, failCount uint32, statusType enterpriseApi.AppPhaseStatusType) {
	logger := logging.FromContext(ctx).With("func", "updatePplnWorkerPhaseInfo")
	logger.InfoContext(ctx, "changing the status", "appName", appDeployInfo.AppName, "old status", appPhaseStatusAsStr(appDeployInfo.PhaseInfo.Status), "new status", appPhaseStatusAsStr(statusType))
...
func (downloadWorker *PipelineWorker) createDownloadDirOnOperator(ctx context.Context) (string, error) {
	...
	if err != nil {
		logger := logging.FromContext(ctx).With("func", "createDownloadDirOnOperator")
		logger.ErrorContext(ctx, "unable to create app download directory on operator", "appSrcName", downloadWorker.appSrcName, "appName", downloadWorker.appDeployInfo.AppName, "error", err)
	}
...
func (downloadWorker *PipelineWorker) download(ctx context.Context, pplnPhase *PipelinePhase, remoteDataClientMgr RemoteDataClientManager, localPath string, downloadWorkersRunPool chan struct{}) {
	...
	logger := logging.FromContext(ctx).With("func", "download", "App name", downloadWorker.appDeployInfo.AppName)
	...
	if err != nil {
		logger.ErrorContext(ctx, "unable to get remote object key", "appName", appName, "error", err)
	...
	if err != nil {
		logger.ErrorContext(ctx, "unable to download app", "appName", appName, "error", err)

		// remove the local file
		err = os.RemoveAll(localFile)
		if err != nil {
			logger.ErrorContext(ctx, "unable to remove local file from operator", "error", err)
		}
	...
	logger.InfoContext(ctx, "Finished downloading app")
}
...
func (pplnPhase *PipelinePhase) downloadWorkerHandler(ctx context.Context, ppln *AppInstallPipeline, maxWorkers uint64, scheduleDownloadsWaiter *sync.WaitGroup) {
	...
	logger := logging.FromContext(ctx).With("func", "downloadWorkerHandler")
	...
	for {
		...
		case downloadWorkersRunPool <- struct{}{}:
			...
			case downloadWorker, ok := <-pplnPhase.msgChannel:
				...
				if !ok {
					logger.InfoContext(ctx, "msgChannel is closed by downloadPhaseManager, hence nothing to read.")
					break downloadWork
				}

				// do not redownload the app if it is already downloaded
				if isAppAlreadyDownloaded(ctx, downloadWorker) {
					logger.InfoContext(ctx, "app is already downloaded on operator pod, hence skipping it.", "appSrcName", downloadWorker.appSrcName, "appName", downloadWorker.appDeployInfo.AppName)
				...
				if err != nil {
					logger.ErrorContext(ctx, "insufficient storage for the app pkg download", "appSrcName", downloadWorker.appSrcName, "appName", downloadWorker.appDeployInfo.AppName, "appSize", downloadWorker.appDeployInfo.Size, "error", err)
				...
				if err != nil {
					logger.ErrorContext(ctx, "unable to create download directory on operator", "appSrcName", downloadWorker.appSrcName, "appName", appDeployInfo.AppName, "error", err)
				...
				if err != nil {
					logger.ErrorContext(ctx, "unable to get remote data client manager", "error", err)
		...
		default:
			...
			logger.DebugContext(ctx, "All the workers are busy, we will check again after one second")
...
func (ppln *AppInstallPipeline) shutdownPipelinePhase(ctx context.Context, phaseManager string, pplnPhase *PipelinePhase, perPhaseWaiter *sync.WaitGroup) {
	logger := logging.FromContext(ctx).With("func", phaseManager)

	// close the msgChannel
	close(pplnPhase.msgChannel)

	// wait for the handler code to finish its work
	logger.InfoContext(ctx, "Waiting for the workers to finish")
	perPhaseWaiter.Wait()

	// mark the phase as done/complete
	logger.InfoContext(ctx, "All the workers finished")
...
func (ppln *AppInstallPipeline) downloadPhaseManager(ctx context.Context) {
	logger := logging.FromContext(ctx).With("func", "downloadPhaseManager")
	logger.InfoContext(ctx, "Starting Download phase manager")
...
downloadPhase:
	for {
		...
		case _, channelOpen := <-ppln.sigTerm:
			if !channelOpen {
				logger.InfoContext(ctx, "Received the termination request from the scheduler")
				break downloadPhase
			}
			...
			for _, downloadWorker := range pplnPhase.q {
				...
				if isPhaseMaxRetriesReached(ctx, phaseInfo, downloadWorker.afwConfig) {
					...
					case pplnPhase.msgChannel <- downloadWorker:
						logger.InfoContext(ctx, "Download worker got a run slot", "name", downloadWorker.cr.GetName(), "namespace", downloadWorker.cr.GetNamespace(), "App name", downloadWorker.appDeployInfo.AppName, "digest", downloadWorker.appDeployInfo.ObjectHash)
...
func markWorkerPhaseInstallationComplete(ctx context.Context, phaseInfo *enterpriseApi.PhaseInfo, worker *PipelineWorker) {
	logger := logging.FromContext(ctx).With("func", "markWorkerPhaseInstallationComplete")
	...
	if isFanOutApplicableToCR(worker.cr) {
		if isAppInstallationCompleteOnAllReplicas(worker.appDeployInfo.AuxPhaseInfo) {
			logger.InfoContext(ctx, "app pkg installed on all the pods", "app pkg", worker.appDeployInfo.AppName)
...
func installApp(rctx context.Context, localCtx *localScopePlaybookContext, cr splcommon.MetaObject, phaseInfo *enterpriseApi.PhaseInfo) error {
	...
	logger := logging.FromContext(rctx).With("func", "installApp", "pod", worker.targetPodName, "app name", worker.appDeployInfo.AppName)
	...
	if !checkIfFileExistsOnPod(rctx, cr, appPkgPathOnPod, localCtx.podExecClient) {
		logger.ErrorContext(rctx, "app pkg missing on Pod", "app pkg path", appPkgPathOnPod)
	...
	if worker.appDeployInfo.AppPackageTopFolder == "" {
		...
		if err != nil {
			logger.ErrorContext(rctx, "local scoped app package install failed while getting name of installed app", "error", err)
			return err
		}
		logger.InfoContext(rctx, "app top folder", "name", appTopFolder)
	...
	if worker.appDeployInfo.IsUpdate {
		...
		logger.InfoContext(rctx, "Check if app is already installed", "name", worker.appDeployInfo.AppPackageTopFolder)

		appInstalled, err := isAppAlreadyInstalled(rctx, cr, localCtx.podExecClient, worker.appDeployInfo.AppPackageTopFolder)
		if err != nil {
			logger.ErrorContext(rctx, "local scoped app package install failed while checking if app is already installed", "error", err)
			return err
		}

		if appInstalled {
			logger.InfoContext(rctx, "Not reinstalling app as it is already installed.")
			return nil
		}
	...
	if stdErr != "" {
		logger.InfoContext(rctx, "App install command stderr output (informational only)", "stderr", stdErr)
	}

	// Check only the actual command execution error, not stderr content
	if err != nil {
		phaseInfo.FailCount++
		logger.ErrorContext(rctx, "local scoped app package install failed", "stdout", stdOut, "stderr", stdErr, "app pkg path", appPkgPathOnPod, "failCount", phaseInfo.FailCount, "error", err)
...
func isAppAlreadyInstalled(ctx context.Context, cr splcommon.MetaObject, podExecClient splutil.PodExecClientImpl, appTopFolder string) (bool, error) {
	logger := logging.FromContext(ctx).With("func", "isAppAlreadyInstalled", "AppTopFolder", appTopFolder)

	logger.DebugContext(ctx, "check app's installation state")
	...
	if stdErr != "" {
		logger.InfoContext(ctx, "Command stderr output (informational only)", "stderr", stdErr)
	}
	...
	if err != nil {
		...
		if strings.Contains(errMsg, "exit status 1") || strings.Contains(errMsg, "command terminated with exit code 1") {
			...
			logger.InfoContext(ctx, "App not enabled - grep pattern not found", "stdout", stdOut, "stderr", stdErr)
			return false, nil
		}
	...
	logger.InfoContext(ctx, "App installation state check successful - app is enabled", "appStatus", strings.TrimSpace(stdOut))
	return true, nil
}
...
func getAppTopFolderFromPackage(rctx context.Context, cr splcommon.MetaObject, appPkgPathOnPod string, podExecClient splutil.PodExecClientImpl) (string, error) {
	logger := logging.FromContext(rctx).With("func", "getAppTopFolderFromPackage", "appPkgPathOnPod", appPkgPathOnPod)
	...
	logger.InfoContext(rctx, "Pod exec result", "stdOut", stdOut)
	...
	if stdErr != "" || err != nil {
		...
		logger.ErrorContext(rctx, "error in tar contents list, but app installation will continue", "stdOut", stdOut, "stdErr", stdErr, "command", command, "error", err)
		if stdOut == "" {
...
func cleanupApp(rctx context.Context, localCtx *localScopePlaybookContext, cr splcommon.MetaObject, phaseInfo *enterpriseApi.PhaseInfo) error {
	...
	logger := logging.FromContext(rctx).With("func", "cleanupApp", "pod", worker.targetPodName, "app name", worker.appDeployInfo.AppName)
	...
	stdOut, stdErr, err := localCtx.podExecClient.RunPodExecCommand(rctx, streamOptions, []string{"/bin/sh"})
	if stdErr != "" || err != nil {
		logger.ErrorContext(rctx, "app pkg deletion failed", "stdout", stdOut, "stderr", stdErr, "app pkg path", appPkgPathOnPod, "error", err)
		return fmt.Errorf("app pkg deletion failed.  stdOut: %s, stdErr: %s, app pkg path: %s", stdOut, stdErr, appPkgPathOnPod)
	}
	logger.InfoContext(rctx, "App package deleted from target pod", "command", command)
...
func (localCtx *localScopePlaybookContext) runPlaybook(rctx context.Context) error {
	worker := localCtx.worker
	cr := worker.cr
	logger := logging.FromContext(rctx).With("func", "runPlaybook", "pod", worker.targetPodName, "app name", worker.appDeployInfo.AppName)
	...
	if err != nil {
		logger.ErrorContext(rctx, "app package installation error", "error", err)
	...
	if err != nil {
		logger.ErrorContext(rctx, "app package cleanup error", "error", err)
...
func extractClusterScopedAppOnPod(ctx context.Context, worker *PipelineWorker, appSrcScope string, appPkgPathOnPod, appPkgLocalPath string, podExecClient splutil.PodExecClientImpl) error {
	logger := logging.FromContext(ctx).With("func", "extractClusterScopedAppOnPod", "app name", worker.appDeployInfo.AppName)
	...
	if clusterAppsPath == "" {
		...
		logger.ErrorContext(ctx, "could not find the cluster scoped apps location on the Pod")
		return err
	}
...
func runPodCopyWorker(ctx context.Context, worker *PipelineWorker, ch chan struct{}) {
	cr := worker.cr
	logger := logging.FromContext(ctx).With("func", "runPodCopyWorker", "app name", worker.appDeployInfo.AppName, "pod", worker.targetPodName)
	defer func() {
		<-ch
		worker.isActive = false
		worker.waiter.Done()
	...
	if err != nil {
		...
		logger.ErrorContext(ctx, "app package is missing", "pod name", worker.targetPodName, "error", err)
	...
	if err != nil {
		phaseInfo.FailCount++
		logger.ErrorContext(ctx, "app package pod copy failed", "stdout", stdOut, "stderr", stdErr, "failCount", phaseInfo.FailCount, "error", err)
		return
	}
	...
	if appSrcScope == enterpriseApi.ScopeCluster {
		...
		if err != nil {
			phaseInfo.FailCount++
			logger.ErrorContext(ctx, "extracting the app package on pod failed", "failCount", phaseInfo.FailCount, "error", err)
			return
		}
	}

	logger.InfoContext(ctx, "podCopy complete", "app pkg path", appPkgPathOnPod)
...
func (pplnPhase *PipelinePhase) podCopyWorkerHandler(ctx context.Context, handlerWaiter *sync.WaitGroup, numPodCopyRunners int) {
	logger := logging.FromContext(ctx).With("func", "podCopyWorkerHandler")
...
podCopyHandler:
		...
		case podCopyWorkerPool <- struct{}{}:
			...
			case worker, channelOpen := <-pplnPhase.msgChannel:
				if !channelOpen {
					...
					logger.InfoContext(ctx, "worker channel closed")
				...
				if worker != nil {
					...
					/// This should never happen
					logger.ErrorContext(ctx, "invalid worker reference")
					<-podCopyWorkerPool
				}
	...
	logger.InfoContext(ctx, "Waiting for all the workers to finish")
	pplnPhase.workerWaiter.Wait()
	logger.InfoContext(ctx, "All the workers finished")
}
...
func (ppln *AppInstallPipeline) podCopyPhaseManager(ctx context.Context) {
	logger := logging.FromContext(ctx).With("func", "podCopyPhaseManager")
	logger.InfoContext(ctx, "Starting Pod copy phase manager")
			...
			for _, podCopyWorker := range pplnPhase.q {
				...
				if isPhaseMaxRetriesReached(ctx, phaseInfo, podCopyWorker.afwConfig) {
					...
					case pplnPhase.msgChannel <- podCopyWorker:
						logger.InfoContext(ctx, "Pod copy worker got a run slot", "name", podCopyWorker.cr.GetName(), "namespace", podCopyWorker.cr.GetNamespace(), "pod name", podCopyWorker.targetPodName, "App name", podCopyWorker.appDeployInfo.AppName, "digest", podCopyWorker.appDeployInfo.ObjectHash)
...
func getInstallSlotForPod(ctx context.Context, installTracker []chan struct{}, podName string) bool {
	logger := logging.FromContext(ctx).With("func", "getInstallSlotForPod")
	podID, err := getOrdinalValFromPodName(podName)
	if err != nil {
		logger.ErrorContext(ctx, "unable to derive podId for podname", "podName", podName, "error", err)
		return false
	}
...
func freeInstallSlotForPod(ctx context.Context, installTracker []chan struct{}, podName string) {
	logger := logging.FromContext(ctx).With("func", "freeInstallSlotForPod")
	podID, err := getOrdinalValFromPodName(podName)
	if err != nil {
		logger.ErrorContext(ctx, "unable to derive podId for podname", "podName", podName, "error", err)
		return
	...
	default:
		logger.ErrorContext(ctx, "trying to free an install slot without even allocating it")
	}
}
...
func (pplnPhase *PipelinePhase) installWorkerHandler(ctx context.Context, ppln *AppInstallPipeline, handlerWaiter *sync.WaitGroup, installTracker []chan struct{}) {
	logger := logging.FromContext(ctx).With("func", "installWorkerHandler")
	defer handlerWaiter.Done()
	...
	for {
		...
		case installWorker, channelOpen := <-pplnPhase.msgChannel:
			...
			if installWorker != nil {
				...
				if err != nil {
					logger.ErrorContext(ctx, "getting app source spec failed while installing app", "appSrcName", installWorker.appSrcName, "error", err)
				}
				...
				if iwctx != nil {
					...
					logger.ErrorContext(ctx, "unable to get install worker context", "app name", installWorker.appDeployInfo.AppName)
				}
			} else {
				// This should never happen
				logger.ErrorContext(ctx, "invalid worker reference")
...
func (ppln *AppInstallPipeline) installPhaseManager(ctx context.Context) {
	logger := logging.FromContext(ctx).With("func", "installPhaseManager")
	logger.InfoContext(ctx, "Starting Install phase manager")
		...
		default:
			for _, installWorker := range pplnPhase.q {
				...
				if !canAppScopeHaveInstallWorker(appScope) {
					logger.ErrorContext(ctx, "Install worker with incorrect scope", "name", installWorker.cr.GetName(), "namespace", installWorker.cr.GetNamespace(), "pod name", installWorker.targetPodName, "App name", installWorker.appDeployInfo.AppName, "digest", installWorker.appDeployInfo.ObjectHash, "scope", appScope)
					continue
				}
				...
				if isPhaseMaxRetriesReached(ctx, phaseInfo, installWorker.afwConfig) {
					...
					if isFanOutApplicableToCR(installWorker.cr) {
						logger.InfoContext(ctx, "Max retries reached for fanout CR - updating main phase info", "app", installWorker.appDeployInfo.AppName, "failCount", phaseInfo.FailCount)
					...
					case pplnPhase.msgChannel <- installWorker:
						logger.InfoContext(ctx, "Install worker got a run slot", "name", installWorker.cr.GetName(), "namespace", installWorker.cr.GetNamespace(), "pod name", installWorker.targetPodName, "App name", installWorker.appDeployInfo.AppName, "digest", installWorker.appDeployInfo.ObjectHash)
...
func validatePhaseInfo(ctx context.Context, phaseInfo *enterpriseApi.PhaseInfo) bool {
	logger := logging.FromContext(ctx).With("func", "validatePhaseInfo")

	// Check for phase in phaseInfo
	phases := string(
		enterpriseApi.PhaseDownload) + string(
		enterpriseApi.PhasePodCopy) + string(
		enterpriseApi.PhaseInstall)

	if !strings.Contains(phases, string(phaseInfo.Phase)) {
		logger.ErrorContext(ctx, "Invalid phase in PhaseInfo", "phase", string(phaseInfo.Phase))
		return false
	}

	if ok := appPhaseInfoStatuses[phaseInfo.Status]; !ok {
		logger.ErrorContext(ctx, "Invalid status in PhaseInfo", "phase", string(phaseInfo.Phase), "status", phaseInfo.Status)
		return false
	}
	return true
}
...
func deleteAppPkgFromOperator(ctx context.Context, worker *PipelineWorker) {
	logger := logging.FromContext(ctx).With("func", "deleteAppPkgFromOperator", "app pkg", worker.appDeployInfo.AppName)

	appPkgLocalPath := getAppPackageLocalPath(ctx, worker)
	err := os.Remove(appPkgLocalPath)
	if err != nil {
		logger.ErrorContext(ctx, "failed to delete app pkg from Operator", "app pkg path", appPkgLocalPath, "error", err)
		return
	}

	logger.InfoContext(ctx, "Deleted app package from the operator", "App package path", appPkgLocalPath)
...
func afwGetReleventStatefulsetByKind(ctx context.Context, cr splcommon.MetaObject, client splcommon.ControllerClient) *appsv1.StatefulSet {
	logger := logging.FromContext(ctx).With("func", "afwGetReleventStatefulsetByKind")
	...
	if err != nil {
		logger.ErrorContext(ctx, "Unable to get the stateful set", "error", err)
	}
...
func getInsallWorkerPlaybookContext(ctx context.Context, worker *PipelineWorker, sem chan struct{}, podExecClient splutil.PodExecClientImpl, appSrcSpec *enterpriseApi.AppSourceSpec, appSrcScope string, ppln *AppInstallPipeline) PlaybookImpl {
	logger := logging.FromContext(ctx).With("func", "getInsallWorkerPlaybookContext")
	...
	// Invalid scope
	logger.ErrorContext(ctx, "Install workers can have only local or premium apps scope", "appSrcScope", appSrcScope)
...
func (shcPlaybookContext *SHCPlaybookContext) isBundlePushComplete(ctx context.Context) (bool, error) {
	logger := logging.FromContext(ctx).With("func", "isBundlePushComplete")
	...
	if stdOut == "" {
		logger.InfoContext(ctx, "SHC Bundle Push is still in progress")
		return false, nil
		...
		logger.ErrorContext(ctx, "SHC Bundle push status file reported an error while applying bundle", "error", err)
	...
	if err != nil {
		logger.ErrorContext(ctx, "removing SHC Bundle Push status file failed, will retry again.", "error", err)
...
func (shcPlaybookContext *SHCPlaybookContext) triggerBundlePush(ctx context.Context) error {
	...
	logger.InfoContext(ctx, "Triggering bundle push", "command", cmd)
	streamOptions := splutil.NewStreamOptionsObject(cmd)
	stdOut, stdErr, err := shcPlaybookContext.podExecClient.RunPodExecCommand(ctx, streamOptions, []string{"/bin/sh"})
	if err != nil || stdErr != "" {
...
func (shcPlaybookContext *SHCPlaybookContext) setLivenessProbeLevel(ctx context.Context, probeLevel int) error {
	logger := logging.FromContext(ctx).With("func", "setLivenessProbeLevel")
	...
	err = func() error {
		...
		if err != nil {
			logger.ErrorContext(ctx, "Unable to set the Liveness probe level", "error", err)
			return err
		}
...
func (shcPlaybookContext *SHCPlaybookContext) runPlaybook(ctx context.Context) error {
	logger := logging.FromContext(ctx).With("func", "SHCPlaybookContext.runPlaybook")
	...
	if cr.Status.Phase != enterpriseApi.PhaseReady {
		logger.InfoContext(ctx, "SHC is not ready yet.")
		return nil
	}
	...
	case enterpriseApi.BundlePushInProgress:
		logger.InfoContext(ctx, "checking the status of SHC Bundle Push")
		// check if the bundle push is complete
		ok, err = shcPlaybookContext.isBundlePushComplete(ctx)
		if ok {
			logger.InfoContext(ctx, "Bundle push complete, setting bundle push state in CR")
			...
			logger.ErrorContext(ctx, "there was an error in SHC bundle push, will retry again", "error", err)
		} else {
			logger.InfoContext(ctx, "SHC Bundle Push is still in progress, will check back again")
		}
	case enterpriseApi.BundlePushPending:
		// run the command to apply cluster bundle
		logger.InfoContext(ctx, "running command to apply SHC Bundle")

		// Adjust the file permissions
		err = adjustClusterAppsFilePermissions(ctx, shcPlaybookContext.podExecClient)
		if err != nil {
			logger.ErrorContext(ctx, "failed to adjust the file permissions", "error", err)
			return err
		}

		err = shcPlaybookContext.triggerBundlePush(ctx)
		if err != nil {
			logger.ErrorContext(ctx, "failed to apply SHC Bundle", "error", err)
			return err
		}

		logger.InfoContext(ctx, "SHC Bundle Push is in progress")
...
func (idxcPlaybookContext *IdxcPlaybookContext) isBundlePushComplete(ctx context.Context) bool {
	logger := logging.FromContext(ctx).With("func", "IdxcPlaybookContext.isBundlePushComplete")
	...
	if err == nil && strings.Contains(stdOut, "cluster_status=None") && !strings.Contains(stdOut, "last_bundle_validation_status=failure") {
		logger.InfoContext(ctx, "IndexerCluster Bundle push complete")
		return true
	}

	if err != nil || stdErr != "" {
		logger.ErrorContext(ctx, "show cluster-bundle-status failed", "stdout", stdOut, "stderr", stdErr, "error", err)
		return false
	}

	logger.InfoContext(ctx, "IndexerCluster Bundle push is still in progress")
	return false
}
...
func (idxcPlaybookContext *IdxcPlaybookContext) triggerBundlePush(ctx context.Context) error {
	logger := logging.FromContext(ctx).With("func", "IdxcPlaybookContext.triggerBundlePush")
	...
	if strings.Contains(stdErr, idxcBundleAlreadyPresentStr) {
		logger.InfoContext(ctx, "bundle already present on peers")
...
func (idxcPlaybookContext *IdxcPlaybookContext) setLivenessProbeLevel(ctx context.Context, probeLevel int) error {
	logger := logging.FromContext(ctx).With("func", "IdxcPlaybookContext.setLivenessProbeLevel")
	var err error
	...
	err = func() error {
		...
		for i := 0; i < len(managerOwnerRefs); i++ {
			...
			if err != nil {
				...
				logger.ErrorContext(ctx, "Unable to fetch the CR", "Name", managerOwnerRefs[i].Name, "Namespace", idxcPlaybookContext.cr.GetNamespace(), "error", err)
				continue
			}
...
func (idxcPlaybookContext *IdxcPlaybookContext) runPlaybook(ctx context.Context) error {
	...
	logger := logging.FromContext(ctx).With("func", "IdxcPlaybookContext.runPlaybook")
	...
	// if the bundle push is already in progress, check the status
	case enterpriseApi.BundlePushInProgress:
		logger.InfoContext(ctx, "checking the status of IndexerCluster Bundle Push")
		// check if the bundle push is complete
		if idxcPlaybookContext.isBundlePushComplete(ctx) {
			...
			logger.InfoContext(ctx, "IndexerCluster Bundle Push is still in progress, will check back again in next reconcile..")
		}
	...
	case enterpriseApi.BundlePushPending:
		...
		// run the command to apply cluster bundle
		logger.InfoContext(ctx, "running command to apply IndexerCluster Bundle")
		err = idxcPlaybookContext.triggerBundlePush(ctx)
		if err != nil {
			logger.ErrorContext(ctx, "failed to apply IndexerCluster Bundle", "error", err)
			return err
		}
...
func handleEsappPostinstall(rctx context.Context, preCtx *premiumAppScopePlaybookContext, phaseInfo *enterpriseApi.PhaseInfo) error {
	...
	logger := logging.FromContext(rctx).With("func", "handleEsappPostinstall", "pod", worker.targetPodName, "app name", worker.appDeployInfo.AppName)
	...
	if stdErr != "" || err != nil {
		phaseInfo.FailCount++
		logger.ErrorContext(rctx, "premium scoped app package install failed", "stdout", stdOut, "stderr", stdErr, "post install command", command, "failCount", phaseInfo.FailCount, "error", err)
...
func (preCtx *premiumAppScopePlaybookContext) runPlaybook(rctx context.Context) error {
	...
	logger := logging.FromContext(rctx).With("func", "premiumAppScopePlaybookContext.runPlaybook", "pod", worker.targetPodName, "app name", worker.appDeployInfo.AppName)
	...
	if err != nil {
		logger.ErrorContext(rctx, "premium app package installation error", "error", err)
		return fmt.Errorf("app pkg installation failed. error %s", err.Error())
	}
	...
	if appSrcSpec.PremiumAppsProps.Type == enterpriseApi.PremiumAppsTypeEs {
		...
		if err != nil {
			logger.ErrorContext(rctx, "app package post installation error", "error", err)
	...
	if err != nil {
		logger.ErrorContext(rctx, "premium app package cleanup error", "error", err)
		return fmt.Errorf("app pkg cleanup failed. error %s", err.Error())
	}
...
func afwSchedulerEntry(ctx context.Context, client splcommon.ControllerClient, cr splcommon.MetaObject, appDeployContext *enterpriseApi.AppDeploymentContext, appFrameworkConfig *enterpriseApi.AppFrameworkSpec) (bool, error) {
	logger := logging.FromContext(ctx).With("func", "afwSchedulerEntry")
	...
	logger.InfoContext(ctx, "Creating pipeline workers for pending app packages")
	...
	for appSrcName, appSrcDeployInfo := range appDeployContext.AppsSrcDeployStatus {
		...
		if err != nil {
			logger.ErrorContext(ctx, "unable to create directory on splunk pod", "error", err)
	...
	logger.InfoContext(ctx, "Waiting for the phase managers to finish")

	// Wait for all the pipeline managers to finish
	afwPipeline.phaseWaiter.Wait()
	logger.InfoContext(ctx, "All the phase managers finished")
...
func (ppln *AppInstallPipeline) afwYieldWatcher(ctx context.Context) {
	logger := logging.FromContext(ctx).With("func", "afwYieldWatcher")
...
yieldScheduler:
	for {
		select {
		case <-yieldTrigger:
			logger.InfoContext(ctx, "Yielding from AFW scheduler", "time elapsed", time.Now().Unix()-ppln.afwEntryTime)
	...
	close(ppln.sigTerm)
	ppln.phaseWaiter.Done()
	logger.InfoContext(ctx, "Termination issued")
}

...
// (transformed to example.com/* format by SyncParentMetaToStatefulSet) are properly
...
func TestApplyStatefulSet_STSOnlyAnnotationNotPersisted(t *testing.T) {
	...
	// This simulates the state after sts-only.example.com/aaa annotation was transformed
	revised := current.DeepCopy()
	revised.Annotations = map[string]string{
		"existing-annotation": "existing-value",
		"example.com/aaa":     "value1", // Added by SyncParentMetaToStatefulSet (prefix stripped)
	}

	t.Logf("Before ApplyStatefulSet - revised has annotation 'example.com/aaa': %s",
		revised.Annotations["example.com/aaa"])
	...
	t.Logf("After ApplyStatefulSet - revised has annotation 'example.com/aaa': %s",
		revised.Annotations["example.com/aaa"])

	// Verify the annotation is preserved in revised (which now reflects current after merge)
	if val, ok := revised.Annotations["example.com/aaa"]; !ok || val != "value1" {
		t.Errorf("Annotation 'example.com/aaa' was lost after ApplyStatefulSet.\n"+
			"Expected: value1\nGot: %s", val)
	} else {
		t.Log("Annotation 'example.com/aaa' is preserved after ApplyStatefulSet")
	}
}

...
func TestHasExcludedPrefix(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			key:      "sts-only.example.com/priority",
...
func TestStripTargetPrefix(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "strips sts-only prefix with example.com domain",
			key:             "sts-only.example.com/priority",
			prefix:          "sts-only.",
			wantKey:         "example.com/priority",
			wantTransformed: true,
		},
		{
		...
		},
		{
			...
			key:             "sts-only.example.com/priority",
			prefix:          "pod-only.",
			wantKey:         "sts-only.example.com/priority",
			wantTransformed: false,
		},
		{
...
func TestAppendParentMeta_PrefixFiltering(t *testing.T) {
	parent := corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Labels: map[string]string{
				"team":                          "platform",
				"sts-only.example.com/priority": "high", // Should be filtered (StatefulSet-only)
	...
	if _, exists := child.Labels["sts-only.example.com/priority"]; exists {
		t.Errorf("Label 'sts-only.example.com/priority' should have been filtered out")
	}
...
func TestComputeDesiredPodTemplateKeys(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			parentLabels: map[string]string{
				"team":                          "platform",
				"sts-only.example.com/priority": "high",
			},
			parentAnnotations: map[string]string{
				"description":                "test",
...
func TestComputeDesiredStatefulSetKeys(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name: "conflict resolution: prefixed key wins over explicit key (more specific)",
			parentLabels: map[string]string{
				"example.com/priority":          "low",  // explicit
				"sts-only.example.com/priority": "high", // would transform to same key
			},
			parentAnnotations: map[string]string{
			...
			expectedLabels: map[string]string{
				"example.com/priority": "high", // prefixed wins (more specific)
			},
			expectedAnnotations: map[string]string{
...
func TestSyncParentMetaToStatefulSet(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedChildAnnotations: map[string]string{
				"example.com/owner":               "ops",
				ManagedCRLabelKeysAnnotation:      `["example.com/priority"]`,
				ManagedCRAnnotationKeysAnnotation: `["example.com/owner"]`,
			},
			expectedManagedLabels:      []string{"example.com/priority"},
		...
		},
		{
			...
			childAnnotations: map[string]string{},
			parentLabels: map[string]string{
				"example.com/priority":          "low",
				"sts-only.example.com/priority": "high",
			},
			parentAnnotations:   map[string]string{},
			selectorLabels:      selectorLabels,
			expectedChildLabels: map[string]string{"example.com/priority": "high"},
			expectedChildAnnotations: map[string]string{
				ManagedCRLabelKeysAnnotation: `["example.com/priority"]`,
			},
			expectedManagedLabels:      []string{"example.com/priority"},
...
// TestSyncParentMetaToStatefulSet_SelectorLabelProtection verifies that selector labels
// on the StatefulSet are not overwritten by CR metadata with matching keys but different values.
func TestSyncParentMetaToStatefulSet_SelectorLabelProtection(t *testing.T) {
	ctx := context.Background()

	// Selector labels that must be protected
	selectorLabels := map[string]string{
		"app.kubernetes.io/instance": "splunk-test-cr-monitoring-console",
		"app.kubernetes.io/part-of":  "splunk-test-cr-monitoring-console",
		"app.kubernetes.io/name":     "monitoring-console",
	}

	// CR has same keys but different values
	parent := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			Labels: map[string]string{
				"app.kubernetes.io/instance": "monitoring-console", // Different value!
				"app.kubernetes.io/part-of":  "my-app",             // Different value!
				"team":                       "platform",           // Should be added
			},
		},
	}

	// StatefulSet starts with selector label values
	child := &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Labels: map[string]string{
				"app.kubernetes.io/instance": "splunk-test-cr-monitoring-console",
	...
	SyncParentMetaToStatefulSet(ctx, child.GetObjectMeta(), parent.GetObjectMeta(), selectorLabels)

	// Verify selector labels are NOT overwritten
	if child.Labels["app.kubernetes.io/instance"] != "splunk-test-cr-monitoring-console" {
		t.Errorf("Selector label app.kubernetes.io/instance was overwritten: got %q, want %q",
			child.Labels["app.kubernetes.io/instance"], "splunk-test-cr-monitoring-console")
	}
	if child.Labels["app.kubernetes.io/part-of"] != "splunk-test-cr-monitoring-console" {
		t.Errorf("Selector label app.kubernetes.io/part-of was overwritten: got %q, want %q",
			child.Labels["app.kubernetes.io/part-of"], "splunk-test-cr-monitoring-console")
	}

	// Verify non-selector labels ARE added
	if child.Labels["team"] != "platform" {
		t.Errorf("Non-selector label 'team' was not added: got %q, want %q",
			child.Labels["team"], "platform")
	}
}

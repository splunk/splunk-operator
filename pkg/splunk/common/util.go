...
//   - sts-only.example.com/priority: "high"  → appears as example.com/priority: "high" on StatefulSet only
const (
	...
	// Example: "sts-only.example.com/priority" → "example.com/priority"
...
//   - "sts-only.example.com/priority" → "example.com/priority" (for StatefulSet)
//
...
// and prefix transformation (sts-only.* → prefix stripped, e.g., sts-only.example.com/priority → example.com/priority).
//
// Conflict Resolution:
...
func SyncParentMetaToStatefulSet(ctx context.Context, child, parent metav1.Object, selectorLabels map[string]string) {
	...
	// Sync labels: add/update desired keys, but protect selector labels
	for k, v := range desiredLabels {
		// Skip selector labels - these must not be overwritten by CR metadata
		// This prevents the "selector does not match template labels" error

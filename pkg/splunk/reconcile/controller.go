...
type SplunkController interface {
	...
	GetInstance() enterprisev1.MetaObject
	...
	Reconcile(client.Client, enterprisev1.MetaObject) (reconcile.Result, error)
}
...
func AddToManager(mgr manager.Manager, splctrl SplunkController) error {
	...
	c, err := controller.New(instance.GetTypeMeta().Kind, mgr, controller.Options{Reconciler: reconciler})
	if err != nil {
		return err
	}
...
func (r splunkReconciler) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	instance.GetObjectKind().SetGroupVersionKind(gvk)

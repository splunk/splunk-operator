...
func updateCRStatus(ctx context.Context, client splcommon.ControllerClient, origCR *enterpriseApi.LicenseManager, crError *error) {
	...
	for tryCnt = 0; tryCnt < maxRetryCountForCRStatusUpdate; tryCnt++ {
		...
		if err = client.Status().Update(ctx, latestCR); err == nil {
...
func fetchCurrentCRWithStatusUpdate(ctx context.Context, client splcommon.ControllerClient, origCR *enterpriseApi.LicenseManager, crError *error) (*enterpriseApi.LicenseManager, error) {

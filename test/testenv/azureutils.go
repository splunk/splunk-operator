...
// UploadFileFromAzure uploads a file to Azure Storage account container
func UploadFileFromAzure(accountName, accountKey, appPackageFullPath, localFileName string) bool {
...
func ListAppsonAzure(accountName, accountKey, bucketName, pathWithInBucket string) bool {
	...
	if err != nil {
		fmt.Println(err.Error(), "Azure Blob Failed to create request for App fetch URL")
		return false
	}
	...
	if err != nil {
		fmt.Println(err.Error(), "Azure blob with secrets ,Errored when unmarshelling list")
		return false
	}
	...
	fmt.Println("Azure blob with secret key done")
	return true
}

...
// DeleteDeployer delete Deployer in given namespace
func DeleteDeployer(ns string) {
	output, err := exec.Command("kubectl", "delete", "deployer", "-n", ns, "--all").Output()
	if err != nil {
		cmd := fmt.Sprintf("kubectl delete deployer -n %s --all", ns)
		logf.Log.Error(err, "Failed to execute command", "command", cmd)
	} else {
		logf.Log.Info("Deployer deleted", "Namespace", ns, "stdout", output)
	}
}
...
// DeployerInNamespace returns true if Deployer is present in namespace
func DeployerInNamespace(ns string) bool {
	output, err := exec.Command("kubectl", "get", "deployer", "-n", ns).Output()
	deleted := true
	if err != nil {
		cmd := fmt.Sprintf("kubectl get deployer -n %s", ns)
		logf.Log.Error(err, "Failed to execute command", "command", cmd)
		return deleted
	}
	logf.Log.Info("Output of command", "Output", string(output))

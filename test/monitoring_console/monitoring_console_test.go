	...
	XContext("Clustered deployment (C3 - clustered indexer, search head cluster)", func() {
		It("monitoringconsole, smoke: MC can configure SHC, indexer instances after scale up and standalone in a namespace", func() {
			...
			*/
			...
			testenv.VerifyPodsInMCConfigMap(deployment, testenvInstance, []string{fmt.Sprintf(testenv.ClusterMasterServiceName, deployment.GetName())}, "SPLUNK_CLUSTER_MANAGER_URL", mcName, true)
	...
	Context("Clustered deployment (C3 - clustered indexer, search head cluster)", func() {
		It("monitoringconsole, integration: MC can configure SHC, indexer instances and reconfigure to new MC", func() {
			...
			*/
			...
			testenv.VerifyPodsInMCConfigMap(deployment, testenvInstance, []string{fmt.Sprintf(testenv.ClusterMasterServiceName, deployment.GetName())}, "SPLUNK_CLUSTER_MANAGER_URL", mcTwoName, true)
			...
			testenv.VerifyPodsInMCConfigMap(deployment, testenvInstance, []string{fmt.Sprintf(testenv.ClusterMasterServiceName, deployment.GetName())}, "SPLUNK_CLUSTER_MANAGER_URL", mcName, false)

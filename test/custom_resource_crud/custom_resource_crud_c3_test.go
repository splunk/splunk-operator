...
var _ = Describe("Crcrud test for SVA C3", func() {
	...
	Context("Clustered deployment (C3 - clustered indexer, search head cluster)", func() {
		It("mastercrcrud, integration, c3: can deploy indexer and search head cluster, change their CR, update the instances", NodeTimeout(testenv.LongTimeout), func(ctx SpecContext) {
	...
	Context("Clustered deployment (C3 - clustered indexer, search head cluster)", func() {
		It("mastercrcrud, integration, c3: can verify IDXC, CM and SHC PVCs are correctly deleted after the CRs deletion", NodeTimeout(testenv.LongTimeout), func(ctx SpecContext) {
			...
			testcaseEnvInst.VerifyPVCsPerDeployment(deployment, splcommon.ClusterManager, 1, true, verificationTimeout)
			...
			testcaseEnvInst.VerifyPVCsPerDeployment(deployment, splcommon.ClusterManager, 1, false, verificationTimeout)

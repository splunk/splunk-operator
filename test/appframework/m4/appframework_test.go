...
var _ = Describe("m4appfw test", func() {
	...
	Context("Multisite Indexer Cluster with Search Head Cluster (M4) with App Framework", func() {
		It("can deploy a M4 SVA with App Framework enabled, install apps and upgrade them", Label("tier:e2e-pr", "sva:m4", "cloud:aws", "variant:master", "feature:appframework"), NodeTimeout(testenv.MediumTimeout), func(ctx SpecContext) {
			...
			*/
			...
			Expect(err).To(Succeed(), "Unable to snapshot Cluster Master app object hashes before V2 upgrade")
	...
	Context("Multisite Indexer Cluster with Search Head Cluster (m4) with App Framework", func() {
		It("can deploy a M4 SVA with App Framework enabled, install apps and downgrade them", Label("tier:e2e-full", "sva:m4", "cloud:aws", "cloud:gcp", "variant:master", "feature:appframework"), NodeTimeout(testenv.MediumTimeout), func(ctx SpecContext) {
			...
			*/
			...
			oldCMHashes, err := testcaseEnvInst.GetAppObjectHashes(ctx, deployment, cm.Name, cm.Kind, appSourceNameIdxc, appFileList)
			Expect(err).To(Succeed(), "Unable to snapshot Cluster Master app object hashes before V1 downgrade")
			oldSHCHashes, err := testcaseEnvInst.GetAppObjectHashes(ctx, deployment, shc.Name, shc.Kind, appSourceNameShc, appFileList)
	...
	Context("Multi Site Indexer Cluster with Search Head Cluster (m4) with App Framework", func() {
		It("can deploy a M4 SVA and have apps installed and updated locally on Cluster Manager and Deployer via manual poll", Label("tier:e2e-full", "sva:m4", "cloud:aws", "cloud:gcp", "variant:master", "feature:appframework"), NodeTimeout(testenv.MediumTimeout), func(ctx SpecContext) {
			...
			*/
			...
			// Wait for operator to detect new content (ObjectHash flip) and complete installation
			Expect(testcaseEnvInst.WaitForAppContentUpdate(ctx, deployment, cm.Name, cm.Kind, appSourceNameIdxc, appFileList, previousCMAppHashes, testenv.AppInstallTimeout)).To(Succeed(), "New app content not picked up and installed on Cluster Master")
	...
	Context("Multi Site Indexer Cluster with Search Head Cluster (m4) with App Framework", func() {
		It("can deploy a m4 SVA with apps installed locally on Cluster Manager and Deployer, cluster-wide on Peers and Search Heads, then upgrade them via a manual poll", Label("tier:e2e-full", "sva:m4", "cloud:aws", "cloud:gcp", "variant:master", "feature:appframework"), NodeTimeout(testenv.MediumLongTimeout), func(ctx SpecContext) {
			...
			*/
			...
			// Wait for operator to detect new content (ObjectHash flip) and complete installation
			Expect(testcaseEnvInst.WaitForAppContentUpdate(ctx, deployment, cm.Name, cm.Kind, appSourceNameLocalIdxc, localappFileList, oldCMHashes, testenv.AppInstallTimeout)).To(Succeed(), "New app content not picked up and installed on Cluster Master")
	...
	Context("Single Site Indexer Cluster with Search Head Cluster (M4) and App Framework", func() {
		It("can deploy a M4, add new apps to app source while install is in progress and have all apps installed cluster-wide", Label("tier:e2e-pr", "sva:m4", "cloud:aws", "variant:master", "feature:appframework"), NodeTimeout(testenv.MediumTimeout), func(ctx SpecContext) {
	...
	Context("Multisite Indexer Cluster with Search Head Cluster (m4) with App Framework", func() {
		It("can deploy a M4 SVA with App Framework enabled, install an app, then disable it by using a disabled version of the app and then remove it from app source", Label("tier:e2e-full", "sva:m4", "cloud:aws", "cloud:gcp", "variant:master", "feature:appframework"), NodeTimeout(testenv.MediumTimeout), func(ctx SpecContext) {
			...
			*/
			...
			Expect(err).To(Succeed(), "Unable to snapshot Cluster Master app object hashes before disabling app")
			...
			Expect(testcaseEnvInst.WaitForAppContentUpdate(ctx, deployment, cm.Name, cm.Kind, appSourceNameIdxc, appFileName, oldCMHashes, testenv.AppInstallTimeout)).To(Succeed(), "Disabled app content not detected on Cluster Master")

			// Wait for App state to update after config file change
			Expect(testcaseEnvInst.WaitforAppInstallState(ctx, deployment, idxcPodNames, testcaseEnvInst.GetName(), appName, "disabled", true)).To(Succeed(), "App install state did not update to disabled")
	...
	Context("Multi Site Indexer Cluster with Search Head Cluster (M4) with App Framework", func() {
		It("can deploy a M4 SVA, install apps via manual polling, switch to periodic polling, verify apps are not updated before the end of AppsRepoPollInterval, then updated after", Label("tier:e2e-full", "sva:m4", "cloud:aws", "variant:master", "feature:appframework"), NodeTimeout(testenv.MediumTimeout), func(ctx SpecContext) {
	...
	Context("Multisite Indexer Cluster with Search Head Cluster (m4) with App Framework", func() {
		It("can deploy a M4 SVA with App Framework enabled and update apps after app download is completed", Label("tier:e2e-full", "sva:m4", "cloud:aws", "cloud:gcp", "variant:master", "feature:appframework"), NodeTimeout(testenv.MediumTimeout), func(ctx SpecContext) {
			...
			*/
			...
			oldCMHashes, err := testcaseEnvInst.GetAppObjectHashes(ctx, deployment, cm.Name, cm.Kind, appSourceNameIdxc, appFileList)
			Expect(err).To(Succeed(), "Unable to snapshot Cluster Master app object hashes before V2 concurrent upload")
			oldSHCHashes, err := testcaseEnvInst.GetAppObjectHashes(ctx, deployment, shc.Name, shc.Kind, appSourceNameShc, appFileList)
	...
	Context("Multisite Indexer Cluster with Search Head Cluster (m4) with App Framework", func() {
		It("can deploy a M4 SVA with App Framework enabled, install apps and check IsDeploymentInProgress for CM and SHC CR's", Label("tier:e2e-pr", "sva:m4", "cloud:aws", "variant:master", "feature:appframework"), NodeTimeout(testenv.MediumTimeout), func(ctx SpecContext) {

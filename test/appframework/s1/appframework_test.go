...
var _ = Describe("s1appfw test", func() {
	...
	Context("Standalone deployment (S1) with App Framework", func() {
		It("can deploy a Standalone instance with App Framework enabled, install apps then upgrade them", Label("tier:e2e-pr", "sva:s1", "cloud:aws", "cloud:azure", "feature:appframework"), NodeTimeout(testenv.ShortTimeout), func(ctx SpecContext) {
			...
			*/
			...
			appFrameworkSpec.MaxConcurrentAppDownloads = int64(maxConcurrentAppDownloads)
			spec := enterpriseApi.StandaloneSpec{
				CommonSplunkSpec: enterpriseApi.CommonSplunkSpec{
					...
					LivenessProbe:  testenv.AppFrameworkLivenessProbe(),
					ReadinessProbe: testenv.AppFrameworkReadinessProbe(),
					StartupProbe:   testenv.AppFrameworkStartupProbe(),
				},
			...
			uploadedFiles, err = cloudBackend.UploadFiles(ctx, s3TestDir, appFileList, downloadDirV2)
			Expect(err).To(Succeed(), fmt.Sprintf("Unable to upload %s apps to S3 test directory for Standalone", appVersion))
			uploadedApps = append(uploadedApps, uploadedFiles...)
	...
	Context("Standalone deployment (S1) with App Framework", func() {
		It("can deploy a Standalone instance with App Framework enabled, install apps then downgrade them", Label("tier:e2e-pr", "sva:s1", "cloud:aws", "feature:appframework"), NodeTimeout(testenv.ShortTimeout), func(ctx SpecContext) {
	...
	Context("Standalone deployment (S1) with App Framework", func() {
		It("can deploy a Standalone instance with App Framework enabled, install apps, scale up, install apps on new pod, scale down", Label("tier:e2e-pr", "sva:s1", "cloud:aws", "cloud:azure", "feature:appframework"), NodeTimeout(testenv.MediumTimeout), func(ctx SpecContext) {
	...
	Context("Standalone deployment (S1) with App Framework", func() {
		It("can deploy a Standalone instance with App Framework enabled, install apps, scale up, upgrade apps", Label("tier:e2e-full", "sva:s1", "cloud:aws", "cloud:gcp", "feature:appframework"), NodeTimeout(testenv.ShortTimeout), func(ctx SpecContext) {
			...
			*/
			...
			// Snapshot hashes before uploading V2 so WaitForAppContentUpdate can detect the change
			oldHashes, err := testcaseEnvInst.GetAppObjectHashes(ctx, deployment, deployment.GetName(), standalone.Kind, appSourceName, appFileList)
			Expect(err).To(Succeed(), "Unable to snapshot app object hashes before upgrade")
			...
			// Wait for operator to detect the new app content (ObjectHash flip) and complete installation
			Expect(testcaseEnvInst.WaitForAppContentUpdate(ctx, deployment, deployment.GetName(), standalone.Kind, appSourceName, appFileList, oldHashes, testenv.AppInstallTimeout)).To(Succeed(), "New app content not picked up and installed on Standalone")
			// Wait for Standalone to be in READY status
			Eventually(func() error {
				return testcaseEnvInst.VerifyStandaloneReady(ctx, deployment, deployment.GetName(), standalone)
	...
	Context("Standalone deployment (S1) with App Framework", func() {
		It("can deploy a Standalone and have ES app installed", Label("tier:e2e-full", "sva:s1", "cloud:aws", "cloud:gcp", "feature:appframework"), NodeTimeout(testenv.ShortTimeout), func(ctx SpecContext) {
			...
			*/
			...
			appSourceName = "appframework-" + enterpriseApi.ScopeLocal + testenv.RandomDNSName(3)
			appFrameworkSpec := testcaseEnvInst.GenerateAppFrameworkSpec(ctx, appSourceVolumeName, enterpriseApi.ScopePremiumApps, appSourceName, s3TestDir, 60)
	...
	Context("Standalone deployment (S1) with App Framework", func() {
		It("can deploy a Standalone instance with App Framework enabled and install around 350MB of apps at once", Label("tier:e2e-full", "sva:s1", "cloud:aws", "cloud:gcp", "feature:appframework"), NodeTimeout(testenv.ShortTimeout), func(ctx SpecContext) {
	...
	Context("Standalone deployment (S1) with App Framework", func() {
		It("can deploy a standalone instance with App Framework enabled for manual poll", Label("tier:e2e-pr", "sva:s1", "cloud:aws", "feature:appframework"), NodeTimeout(testenv.ShortTimeout), func(ctx SpecContext) {
			...
			*/
			...
			// Create App Framework Spec
			appSourceName = "appframework-" + enterpriseApi.ScopeLocal + testenv.RandomDNSName(3)
			appFrameworkSpec := testcaseEnvInst.GenerateAppFrameworkSpec(ctx, appSourceVolumeName, enterpriseApi.ScopeLocal, appSourceName, s3TestDir, 0)
	...
	Context("Standalone deployment (S1) with App Framework", func() {
		It("can deploy Several standalone CRs in the same namespace with App Framework enabled", Label("tier:e2e-full", "sva:s1", "cloud:aws", "cloud:gcp", "feature:appframework"), NodeTimeout(testenv.ShortTimeout), func(ctx SpecContext) {
			...
			*/
			...
			uploadedFiles, err := cloudBackend.UploadFiles(ctx, s3TestDir, appFileListStandalone1, downloadDirV1)
			Expect(err).To(Succeed(), "Unable to upload apps to S3 test directory")
			uploadedApps = append(uploadedApps, uploadedFiles...)
			...
			uploadedFiles, err = cloudBackend.UploadFiles(ctx, s3TestDirStandalone2, appFileListStandalone2, downloadDirV1)
			Expect(err).To(Succeed(), "Unable to upload apps to S3 test directory")
			uploadedApps = append(uploadedApps, uploadedFiles...)
			...
			appFrameworkSpecStandalone2 := testcaseEnvInst.GenerateAppFrameworkSpec(ctx, appSourceVolumeNameStandalone2, enterpriseApi.ScopeLocal, appSourceNameStandalone2, s3TestDirStandalone2, 60)
			...
			Eventually(func() error {
				return testcaseEnvInst.VerifyStandaloneReady(ctx, deployment, deployment.GetName(), standalone2)
			}, testenv.DefaultTimeout, testenv.PollInterval).Should(Succeed(), "Standalone not ready")
	...
	Context("Standalone deployment (S1) with App Framework", func() {
		It("can add new apps to app source while install is in progress and have all apps installed", Label("tier:e2e-full", "sva:s1", "cloud:aws", "cloud:gcp", "feature:appframework"), NodeTimeout(testenv.LongTimeout), func(ctx SpecContext) {
			...
			*/
			...
			Eventually(func() error {
				return testcaseEnvInst.VerifyAppInstalled(ctx, deployment, testcaseEnvInst.GetName(), []string{standalonePodName}, appList, true, "enabled", false, false)
	...
	Context("Standalone deployment (S1) with App Framework", func() {
		It("Deploy a Standalone instance with App Framework enabled and reset operator pod while app install is in progress", Label("tier:e2e-full", "sva:s1", "cloud:aws", "cloud:gcp", "feature:appframework"), Serial, NodeTimeout(testenv.LongTimeout), func(ctx SpecContext) {
			...
			*/
			...
			// ############ Verify livenessProbe and readinessProbe config object and scripts############
			testcaseEnvInst.Log.Info("Get config map for livenessProbe and readinessProbe")
			err = wait.PollUntilContextTimeout(ctx, testenv.PollInterval, 5*time.Minute, true, func(ctx context.Context) (bool, error) {
				_, getErr := testenv.GetConfigMap(ctx, deployment, testcaseEnvInst.GetName(), ConfigMapName)
				return getErr == nil, nil
			})
			Expect(err).To(Succeed(), "Unable to get config map for livenessProbe and readinessProbe", "ConfigMap name", ConfigMapName)
	...
	Context("Standalone deployment (S1) with App Framework", func() {
		It("can deploy a Standalone instance with App Framework enabled, install an app then disable it and remove it from app source", Label("tier:e2e-full", "sva:s1", "cloud:aws", "cloud:gcp", "feature:appframework"), NodeTimeout(testenv.ShortTimeout), func(ctx SpecContext) {
			...
			*/
			...
			oldHashes, err := testcaseEnvInst.GetAppObjectHashes(ctx, deployment, deployment.GetName(), standalone.Kind, appSourceName, appFileName)
			Expect(err).To(Succeed(), "Unable to snapshot app object hashes before disable")

			// Disable the app
			err = cloudBackend.DisableApps(ctx, downloadDirV1, appFileName, s3TestDir)
			...
			Expect(testcaseEnvInst.WaitForAppContentUpdate(ctx, deployment, deployment.GetName(), standalone.Kind, appSourceName, appFileName, oldHashes, testenv.AppInstallTimeout)).To(Succeed(), "App content change not detected after disable")
			// Wait for Standalone to be in READY status
			Eventually(func() error {
				return testcaseEnvInst.VerifyStandaloneReady(ctx, deployment, deployment.GetName(), standalone)
			}, testenv.DefaultTimeout, testenv.PollInterval).Should(Succeed(), "Standalone not ready")
	...
	Context("Standalone deployment (S1) with App Framework", func() {
		It("can deploy a Standalone instance with App Framework enabled, attempt to update using incorrect S3 credentials", Label("tier:e2e-full", "sva:s1", "cloud:aws", "cloud:gcp", "feature:appframework"), NodeTimeout(testenv.ShortTimeout), func(ctx SpecContext) {
	...
	Context("Standalone deployment (S1) with App Framework", func() {
		It("Deploy a Standalone instance with App Framework enabled and update apps after app download is completed", Label("tier:e2e-full", "sva:s1", "cloud:aws", "cloud:gcp", "feature:appframework"), NodeTimeout(testenv.LongTimeout), func(ctx SpecContext) {
			...
			* Upload updated app to S3 as previous app download is complete
			...
			*/
			...
			// Create App Framework spec for Standalone
			appSourceName = "appframework-" + enterpriseApi.ScopeLocal + testenv.RandomDNSName(3)
			appFrameworkSpec := testcaseEnvInst.GenerateAppFrameworkSpec(ctx, appSourceVolumeName, enterpriseApi.ScopeLocal, appSourceName, s3TestDir, 120)
			spec := enterpriseApi.StandaloneSpec{
				CommonSplunkSpec: enterpriseApi.CommonSplunkSpec{
					Spec: enterpriseApi.Spec{
						ImagePullPolicy: "Always",
			...
			appVersion = "V1"
			Eventually(func() error {
				return testcaseEnvInst.VerifyAppInstalled(ctx, deployment, testcaseEnvInst.GetName(), []string{fmt.Sprintf(testenv.StandalonePod, deployment.GetName(), 0)}, appListV1, false, "enabled", false, false)
			}, testenv.DefaultTimeout, testenv.PollInterval).Should(Succeed(), "App installation verification failed")
	...
	Context("Standalone deployment (S1) with App Framework", func() {
		It("can deploy a Standalone instance and install a bigger volume of apps than the operator PV disk space", Label("tier:e2e-full", "sva:s1", "cloud:aws", "cloud:gcp", "feature:appframework"), NodeTimeout(testenv.ShortTimeout), func(ctx SpecContext) {
			...
			*/
			...
			// Upload apps to S3
			testcaseEnvInst.Log.Info(fmt.Sprintf("Upload %s apps to S3 for Standalone", appVersion))
			uploadedFiles, err := cloudBackend.UploadFiles(ctx, s3TestDir, appFileList, downloadDirPVTestApps)
			Expect(err).To(Succeed(), "Unable to upload apps to S3 test directory")
			uploadedApps = append(uploadedApps, uploadedFiles...)
	...
	Context("Standalone deployment (S1) with App Framework", func() {
		It("Deploy a Standalone instance with App Framework enabled and delete apps from app directory when app download is complete", Label("tier:e2e-full", "sva:s1", "cloud:aws", "cloud:gcp", "feature:appframework"), NodeTimeout(testenv.LongTimeout), func(ctx SpecContext) {
	...
	Context("Standalone deployment (S1) with App Framework", func() {
		It("can deploy a Standalone instance with App Framework enabled, install apps and check isDeploymentInProgress is set for Standaloen and MC CR's", Label("tier:e2e-pr", "sva:s1", "cloud:aws", "feature:appframework"), NodeTimeout(testenv.ShortTimeout), func(ctx SpecContext) {

...
var _ = Describe("c3appfw test", func() {
	...
	Context("Single Site Indexer Cluster with Search Head Cluster (C3) and App Framework", func() {
		It("can deploy a C3 SVA with App Framework enabled, install apps then upgrade them", Label("tier:e2e-pr", "sva:c3", "cloud:aws", "variant:manager", "feature:appframework"), NodeTimeout(testenv.LongTimeout), func(ctx SpecContext) {
			...
			*/
			...
			Expect(err).To(Succeed(), "Unable to snapshot Cluster Manager app object hashes before upgrade")
	...
	Context("Single Site Indexer Cluster with Search Head Cluster (C3) and App Framework and Image Upgrade", func() {
		...
		PIt("can deploy a C3 SVA with App Framework enabled, install apps then upgrade the image and apps", Label("tier:e2e-pr", "sva:c3", "cloud:aws", "variant:manager", "feature:appframework"), NodeTimeout(testenv.LongTimeout), func(ctx SpecContext) {
			...
			}, deployment.GetTimeout(), testenv.PollInterval).Should(Succeed(), "License Manager not ready")
			...
			Eventually(func() error {
				attemptCtx, cancel := context.WithTimeout(ctx, testenv.ReadinessPollTimeout)
				defer cancel()
				return testcaseEnvInst.VerifyClusterManagerReady(attemptCtx, deployment)
			}, deployment.GetTimeout(), testenv.PollInterval).Should(Succeed(), "Cluster Manager not ready")
			...
			// Image upgrades require a longer timeout due to pod restarts: the LM
			// and CM are upgraded sequentially (CM readiness is gated on the LM
			// being Ready first), and each splunkd restart on the new image can
			// take several minutes, so 10m has been observed to expire just before
			// the CM becomes reachable.
			err = testcaseEnvInst.WaitForClusterManagerPhase(ctx, deployment, testcaseEnvInst.GetName(), cm.Name, enterpriseApi.PhaseReady, 15*time.Minute)
	...
	Context("Single Site Indexer Cluster with Search Head Cluster (C3) with App Framework", func() {
		It("can deploy a C3 SVA with App Framework enabled, install apps then downgrade them", Label("tier:e2e-pr", "sva:c3", "cloud:aws", "cloud:azure", "variant:manager", "feature:appframework"), NodeTimeout(testenv.LongTimeout), func(ctx SpecContext) {
			...
			*/
			...
			// Snapshot hashes before uploading new version
			oldCMHashes, err := testcaseEnvInst.GetAppObjectHashes(ctx, deployment, cm.Name, cm.Kind, appSourceNameIdxc, appFileList)
			Expect(err).To(Succeed(), "Unable to snapshot Cluster Manager app object hashes before downgrade")
			oldSHCHashes, err := testcaseEnvInst.GetAppObjectHashes(ctx, deployment, shc.Name, shc.Kind, appSourceNameShc, appFileList)
	...
	Context("Single Site Indexer Cluster with Search Head Cluster (C3) with App Framework", func() {
		It("can deploy a C3 SVA with App Framework enabled, install apps, scale up clusters, install apps on new pods, scale down", Label("tier:e2e-full", "sva:c3", "cloud:aws", "cloud:gcp", "variant:manager", "feature:appframework", "feature:scaling"), NodeTimeout(testenv.LongTimeout), func(ctx SpecContext) {
			...
			*/
			...
			// their phase is validated after VerifyAppFrameworkState below.
			...
			err = testcaseEnvInst.WaitForScaledUp(ctx, deployment, testcaseEnvInst.GetName(), idxcName, 2*time.Minute)
			...
			err = testcaseEnvInst.WaitForScaledDown(ctx, deployment, testcaseEnvInst.GetName(), idxcName, 2*time.Minute)
	...
	Context("Clustered deployment (C3 - clustered indexer, search head cluster)", func() {
		It("can deploy a C3 SVA and have apps installed locally on Cluster Manager and Deployer", Label("tier:e2e-pr", "sva:c3", "cloud:aws", "cloud:azure", "variant:manager", "feature:appframework"), NodeTimeout(testenv.LongTimeout), func(ctx SpecContext) {
	...
	Context("Clustered deployment (C3 - clustered indexer, search head cluster)", func() {
		It("can deploy a C3 SVA with apps installed locally on Cluster Manager and Deployer, cluster-wide on Peers and Search Heads, then upgrade them", Label("tier:e2e-full", "sva:c3", "cloud:aws", "cloud:gcp", "cloud:azure", "variant:manager", "feature:appframework"), NodeTimeout(testenv.MediumLongTimeout), func(ctx SpecContext) {
			...
			*/
			...
			//############### UPGRADE APPS ################
			previousClusterAppHashes, err := testcaseEnvInst.GetAppObjectHashes(ctx, deployment, cmAppSourceInfoCluster.CrName, cm.Kind, appSourceNameClusterIdxc, clusterappFileList)
			...
			// Snapshot local hashes before uploading V2 local apps
			localappFileList = testenv.GetAppFileList(appListLocal)
			oldCMLocalHashes, err := testcaseEnvInst.GetAppObjectHashes(ctx, deployment, cm.Name, cm.Kind, appSourceNameLocalIdxc, localappFileList)
			Expect(err).To(Succeed(), "Unable to snapshot Cluster Manager local app object hashes before V2 upgrade")
			oldSHCLocalHashes, err := testcaseEnvInst.GetAppObjectHashes(ctx, deployment, shc.Name, shc.Kind, appSourceNameLocalShc, localappFileList)
			Expect(err).To(Succeed(), "Unable to snapshot Search Head Cluster local app object hashes before V2 upgrade")
			...
			// Wait for operator to detect new content on local app sources
			Expect(testcaseEnvInst.WaitForAppContentUpdate(ctx, deployment, cm.Name, cm.Kind, appSourceNameLocalIdxc, localappFileList, oldCMLocalHashes, testenv.AppInstallTimeout)).To(Succeed(), "New app content not picked up and installed on Cluster Manager (local)")
			Expect(testcaseEnvInst.WaitForAppContentUpdate(ctx, deployment, shc.Name, shc.Kind, appSourceNameLocalShc, localappFileList, oldSHCLocalHashes, testenv.AppInstallTimeout)).To(Succeed(), "New app content not picked up and installed on Search Head Cluster (local)")
	...
	Context("Clustered deployment (C3 - clustered indexer, search head cluster)", func() {
		It("can deploy a C3 SVA with apps installed locally on Cluster Manager and Deployer, cluster-wide on Peers and Search Heads, then downgrade them", Label("tier:e2e-full", "sva:c3", "cloud:aws", "cloud:gcp", "variant:manager", "feature:appframework"), NodeTimeout(testenv.MediumLongTimeout), func(ctx SpecContext) {
			...
			*/
			...
			// Snapshot local hashes before uploading V1 local apps
			oldCMLocalHashes, err := testcaseEnvInst.GetAppObjectHashes(ctx, deployment, cm.Name, cm.Kind, appSourceNameLocalIdxc, localappFileList)
			Expect(err).To(Succeed(), "Unable to snapshot Cluster Manager local app object hashes before V1 downgrade")
			oldSHCLocalHashes, err := testcaseEnvInst.GetAppObjectHashes(ctx, deployment, shc.Name, shc.Kind, appSourceNameLocalShc, localappFileList)
			Expect(err).To(Succeed(), "Unable to snapshot Search Head Cluster local app object hashes before V1 downgrade")
	...
	Context("Clustered deployment (C3 - clustered indexer, search head cluster)", func() {
		It("can deploy a C3 SVA instance with App Framework enabled and install above 200MB of apps at once", Label("tier:e2e-full", "sva:c3", "cloud:aws", "cloud:gcp", "variant:manager", "feature:appframework"), NodeTimeout(testenv.MediumTimeout), func(ctx SpecContext) {
	...
	Context("Single Site Indexer Cluster with Search Head Cluster (C3) with App Framework", func() {
		It("can deploy a C3 SVA with App Framework enabled for manual update", Label("tier:e2e-full", "sva:c3", "cloud:aws", "cloud:gcp", "variant:manager", "feature:appframework"), NodeTimeout(testenv.LongTimeout), func(ctx SpecContext) {
			/* Test Steps
			   ################## SETUP ####################
			   * Upload V1 apps to S3
	...
	Context("Single Site Indexer Cluster with Search Head Cluster (C3) and App Framework", func() {
		It("can deploy a C3, add new apps to app source while install is in progress and have all apps installed locally on Cluster Manager and Deployer", Label("tier:e2e-full", "sva:c3", "cloud:aws", "cloud:gcp", "variant:manager", "feature:appframework"), NodeTimeout(testenv.LongTimeout), func(ctx SpecContext) {
			...
			*/
			...
			testcaseEnvInst.Log.Info("Deploy Single Site Indexer Cluster with Search Head Cluster")
			indexerReplicas := 3
			cm, _, _, err := deployment.DeploySingleSiteClusterWithGivenAppFrameworkSpec(ctx, deployment.GetName(), indexerReplicas, true, appFrameworkSpecIdxc, appFrameworkSpecShc, "", "")
			Expect(err).To(Succeed(), "Unable to deploy Single Site Indexer Cluster with Search Head Cluster")
	...
	// Vivek need testing
	Context("Single Site Indexer Cluster with Search Head Cluster (C3) and App Framework", func() {
		It("can deploy a C3, add new apps to app source while install is in progress and have all apps installed cluster-wide", Label("tier:e2e-full", "sva:c3", "cloud:aws", "cloud:gcp", "cloud:azure", "variant:manager", "feature:appframework"), NodeTimeout(testenv.LongTimeout), func(ctx SpecContext) {
	...
	// Vivek need testing
	Context("Single Site Indexer Cluster with Search Head Cluster (C3) and App Framework", func() {
		It("can deploy a C3 SVA with App Framework enabled and reset operator pod while app install is in progress", Label("tier:e2e-full", "sva:c3", "cloud:aws", "cloud:gcp", "variant:manager", "feature:appframework"), Serial, NodeTimeout(testenv.LongTimeout), func(ctx SpecContext) {
	...
	Context("Single Site Indexer Cluster with Search Head Cluster (C3) and App Framework", func() {
		It("can deploy a C3 SVA with App Framework enabled and reset operator pod while app download is in progress", Label("tier:e2e-full", "sva:c3", "cloud:aws", "cloud:gcp", "variant:manager", "feature:appframework"), Serial, NodeTimeout(testenv.LongTimeout), func(ctx SpecContext) {
	...
	Context("Single Site Indexer Cluster with Search Head Cluster (C3) and App Framework", func() {
		It("can deploy a C3 SVA with App Framework enabled, install an app, then disable it by using a disabled version of the app and then remove it from app source", Label("tier:e2e-full", "sva:c3", "cloud:aws", "cloud:gcp", "variant:manager", "feature:appframework"), NodeTimeout(testenv.LongTimeout), func(ctx SpecContext) {
	...
	Context("Single Site Indexer Cluster with Search Head Cluster (C3) and App Framework", func() {
		It("can deploy a C3 SVA with App Framework enabled and update apps after app download is completed", Label("tier:e2e-full", "sva:c3", "cloud:aws", "cloud:gcp", "variant:manager", "feature:appframework"), NodeTimeout(testenv.LongTimeout), func(ctx SpecContext) {
			...
			*/
			...
			previousAppHashes, err := testcaseEnvInst.GetAppObjectHashes(ctx, deployment, cm.Name, cm.Kind, appSourceNameIdxc, appFileList)
			...
			// Wait for operator to detect new content (ObjectHash flip) and complete installation
			Expect(testcaseEnvInst.WaitForAppContentUpdate(ctx, deployment, cm.Name, cm.Kind, appSourceNameIdxc, appFileList, previousAppHashes, testenv.AppInstallTimeout)).To(Succeed(), "New app content not picked up and installed on Cluster Manager")
			Expect(testcaseEnvInst.WaitForAppContentUpdate(ctx, deployment, shc.Name, shc.Kind, appSourceNameShc, appFileList, oldSHCHashes, testenv.AppInstallTimeout)).To(Succeed(), "New app content not picked up and installed on Search Head Cluster")
			// Ensure C3 cluster is ready and RF/SF met
			Expect(testcaseEnvInst.VerifyC3ClusterReadyAndRFSF(ctx, deployment, testcaseEnvInst.VerifyClusterManagerReady)).To(Succeed(), "C3 cluster not ready or RF/SF not met")
	...
	Context("Clustered deployment (C3 - clustered indexer, search head cluster)", func() {
		It("can deploy a C3 SVA and install a bigger volume of apps than the operator PV disk space", Label("tier:e2e-full", "sva:c3", "cloud:aws", "cloud:gcp", "variant:manager", "feature:appframework"), NodeTimeout(testenv.MediumTimeout), func(ctx SpecContext) {
	...
	Context("Single Site Indexer Cluster with Search Head Cluster (C3) and App Framework", func() {
		It("can deploy a C3 SVA with App Framework enabled and delete apps from app directory when download is complete", Label("tier:e2e-full", "sva:c3", "cloud:aws", "cloud:gcp", "variant:manager", "feature:appframework"), NodeTimeout(testenv.LongTimeout), func(ctx SpecContext) {
	...
	Context("Single Site Indexer Cluster with Search Head Cluster (C3) and App Framework", func() {
		It("can deploy a C3 SVA with App Framework enabled and check isDeploymentInProgressFlag for CM and SHC CR's", Label("tier:e2e-pr", "sva:c3", "cloud:aws", "variant:manager", "feature:appframework"), NodeTimeout(testenv.LongTimeout), func(ctx SpecContext) {
	...
	Context("Clustered deployment (C3 - clustered indexer, search head cluster)", func() {
		It("can deploy a C3 SVA and a Standalone, then add that Standalone as a Search Head to the cluster", Label("tier:e2e-full", "sva:c3", "cloud:aws", "feature:appframework"), NodeTimeout(testenv.LongTimeout), func(ctx SpecContext) {
	...
	Context("Clustered deployment (C3 - clustered indexer, search head cluster)", func() {
		It("can deploy a C3 SVA and have ES app installed on Search Head Cluster", Label("tier:e2e-full", "sva:c3", "cloud:aws", "cloud:gcp", "variant:manager", "feature:appframework"), NodeTimeout(testenv.MediumTimeout), func(ctx SpecContext) {
			...
			*/
			...
			err = cloudBackend.DownloadFiles(ctx, testenv.AppLocationV1, downloadDirV1, appFileListIdxc)
			Expect(err).To(Succeed(), "Unable to download ES app file from S3")
			...
			uploadedFiles, err = cloudBackend.UploadFiles(ctx, s3TestDirIdxc, appFileListIdxc, downloadDirV1)
			...
			appSourceVolumeNameShc := "appframework-test-volume-shc-" + testenv.RandomDNSName(3)
			appFrameworkSpecShc := testcaseEnvInst.GenerateAppFrameworkSpec(ctx, appSourceVolumeNameShc, enterpriseApi.ScopePremiumApps, appSourceNameShc, s3TestDirShc, 180)
			...
			appSourceNameIdxc = "appframework-idxc-" + enterpriseApi.ScopeCluster + testenv.RandomDNSName(3)
			appSourceVolumeNameIdxc := "appframework-test-volume-idxc-" + testenv.RandomDNSName(3)
			appFrameworkSpecIdxc := testcaseEnvInst.GenerateAppFrameworkSpec(ctx, appSourceVolumeNameIdxc, enterpriseApi.ScopeCluster, appSourceNameIdxc, s3TestDirIdxc, 180)
			...
			// err = cloudBackend.DownloadFiles(ctx, testenv.AppLocationV2, downloadDirV2, appFileList)
			...
			// testcaseEnvInst.WaitforPhaseChange(ctx, deployment, deployment.GetName()+"-shc", shc.Kind, appSourceNameShc, appFileList)

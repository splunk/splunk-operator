...
printf '%s\n' \
  $'timestamp\tphase\tdesired\tpods\tcontainers_ready\tpod_ready\tserving_true\tendpoints\trestarts\tinitialized\tmin_peers\tcaptain_ready\tlast_stable\tpod_detail\tendpoint_pods\tcontainer_states' \
  > "${evidence_file}"
...
for ((sample = 1; sample <= samples; sample++)); do
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    ...
    "${captain_ready}" "${last_stable}" "${pod_detail}" \
    "${endpoint_pods}" "${container_states}" >> "${evidence_file}"

  if ((desired > 0 && endpoint_count > 0 && containers_ready < desired)); then
    echo "FAIL: Service endpoints appeared before every desired container was ready" >&2
    exit 1
  fi
  if ((desired > 0 && serving_true > 0 && containers_ready < desired)); then
